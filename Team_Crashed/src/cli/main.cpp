...
static Status runStatement(executor::QueryEngine& qe,
    ...
    if (head == "SHOW") {
        ...
        if (upper.find("SHOW") != std::string::npos &&
            upper.find("TABLES") != std::string::npos) {
            outRows.clear();
            if (cat == nullptr) return Status::INVALID_ARGUMENT;
            for (const auto& table : cat->listTables()) {
                executor::Tuple row;
                row.values.push_back(executor::Value::makeStr(table));
...
static int runInteractive(executor::QueryEngine& qe, catalog::CatalogManager* cat) {
    ...
    while (std::getline(std::cin, line)) {
        ...
        try {
            ...
            Status st = runStatement(qe, cat, stmt, rows);
            if (st == Status::OK) {
                if (rows.empty()) {
...
static int runFile(executor::QueryEngine& qe, catalog::CatalogManager* cat,
    ...
    for (char c : script) {
        if (c == ';') {
            ...
            if (!trimmed.empty() && trimmed != "\\q") {
                try {
                    std::vector<executor::Tuple> rows;
                    Status st = runStatement(qe, cat, trimmed + ";", rows);
                    if (st == Status::OK) {
                        if (rows.empty()) {
