...
//   [ dirty     : u8     ] @ 16
//   [ nextPage  : u32    ] @ 20   (heap-file chain pointer; 0 = end of chain)
//   [ padding   : 40*u8  ] @ 24
...
//
// WHY nextPage lives at offset 20: the previous layout reserved a 47-byte
// padding block from 17..63 to round the header to 64 bytes. We carve out
// 4 bytes there for the heap-file chain pointer so the total header size
// stays 64 bytes (no shift of the slot directory, no on-disk migration).
// The catalog page (id 0) does not use this field; it stays zeroed.
// =============================================================================
...
constexpr std::size_t OFF_NEXT_PAGE  = 20;   // heap-file chain pointer
...
// Heap-file chain pointer. 0 means "end of chain". Lives in the page
// header padding at offset OFF_NEXT_PAGE. The catalog page does not use
// this field and always has it 0; for pages written before this layout
// was added the field is zero-initialised on first read, which is fine
// for single-page tables.
PageId Page::nextPage() const {
    return readAt<PageId>(buf_.data(), OFF_NEXT_PAGE);
}

void Page::setNextPage(PageId id) {
    writeAt<PageId>(buf_.data(), OFF_NEXT_PAGE, id);
}
