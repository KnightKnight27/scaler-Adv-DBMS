...
// bootstrap page and is always present. Page id 1 is reserved for index
// metadata, so heap/index allocations begin at page id 2.
//
...
Status DiskManager::freePage(PageId pageId) {
    std::lock_guard<std::mutex> lk(mu_);
    if (pageId == 0 || pageId == 1) return Status::INVALID_ARGUMENT;
