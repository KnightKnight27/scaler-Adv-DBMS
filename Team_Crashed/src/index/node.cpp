...
// Entries are packed with variable-length keys, so this walks the prior
// entries instead of multiplying by a fixed slot width.
inline std::size_t entryOffset(const std::uint8_t* buf, bool isLeaf, std::uint16_t i) {
    if (isLeaf) {
        const std::uint8_t* p = buf + OFF_DATA_BASE;
...
std::span<const std::uint8_t> Node::key(std::uint16_t i) const {
    if (i >= numKeys()) return {};
    const std::uint8_t* p = buf + entryOffset(buf, isLeaf(), i);
    std::uint16_t kLen;
    if (isLeaf()) {
        kLen = readU16(p);
        return { p + 2, kLen };
...
std::span<std::uint8_t> Node::key(std::uint16_t i) {
    if (i >= numKeys()) return {};
    std::uint8_t* p = buf + entryOffset(buf, isLeaf(), i);
    std::uint16_t kLen;
    if (isLeaf()) {
        kLen = readU16(p);
        return { p + 2, kLen };
