...
std::unique_ptr<PhysicalPlan> clonePlan(const PhysicalPlan& p) {
    ...
    out->projectionExprs.reserve(p.projectionExprs.size());
    for (const auto& e : p.projectionExprs) {
        if (e) out->projectionExprs.push_back(cloneExpr(*e));
        else   out->projectionExprs.push_back(nullptr);
    }
