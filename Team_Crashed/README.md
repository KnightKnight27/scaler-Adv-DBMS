...
pluggable **extension track** (Track B: concurrency, see below).
...
**Chosen extension track.** **Track B — Concurrency.** MiniDB keeps strict 2PL
as the serializable baseline and adds MVCC-style snapshot metadata plus
write-write conflict detection for the extension path. The lock manager is
still available for baseline benchmarking and deadlock demonstrations.
...
- **Track B extension:** snapshot high-water marks and write-write conflict
  detection at commit time. `LockManager` is kept for the baseline run.
- Isolation guarantee: `Serializable` under the 2PL path. The MVCC extension
  demonstrates non-locking conflict detection but does not yet implement full
  row-version garbage collection.
...
**Benchmark.** `benchmark/write_benchmark.cpp` compares inserts bracketed by
2PL exclusive locks against inserts through the non-locking MVCC-style path.
The benchmark report is in `docs/benchmark_report.md`.
...
| Member | Roll Number | Scaler Email |
|---|---|---|
...
- Executor-level WAL logging for every INSERT/DELETE is not fully integrated;
  WAL and startup recovery are implemented and tested as standalone modules.
- B+ tree leaf split works for the implemented path, but internal-node
  recursive splitting is still bounded and may return `FULL` for very large
  indexes.
