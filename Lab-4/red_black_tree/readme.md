...
```text
     ...
     5:R  15:R           70:R
```
Notice how the invariants are strictly maintained: no Red node has a Red child, and every path from the root to a leaf has exactly three Black nodes.
...
Because of invariants 4 and 5, the maximum height of a Red-Black tree is capped at `2 * log2(n + 1)`, ensuring strictly logarithmic runtime bounds for all dynamic set operations.
