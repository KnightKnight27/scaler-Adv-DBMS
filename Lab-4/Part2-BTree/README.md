...
> Part 2 of Lab 4 (*Red-Black Tree & Full B-Tree*). The Red-Black Tree is
> in `../Part1-RBT/`.
...
```bash
cd Lab-4/Part2-BTree
g++ -std=c++17 -Wall -Wextra -Wpedantic -O2 btree.cpp -o btree
./btree
```

The binary is not checked in — `.gitignore` excludes the compiled
`btree` binary. If you don't have g++ on PATH any C++17 compiler
...
An earlier version of `eraseFrom` had a bug in the case-3 descent: when a
child with `t-1` keys was **merged with its left sibling**, the target
node moved from index `i` to index `i-1`, but the recursion still
descended into index `i` (the wrong node). The result was that some keys
were never actually removed. The fix makes `ensureChildHasEnough` return
the index the target lives at after a borrow/merge, and the caller
descends into that returned index. Verified with a fuzz test of 40 seeds
× 5 degrees × 8000 mixed ops each against `std::map`.
