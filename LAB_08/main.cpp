...
// Student: prince kumar(23bcs10071)
...
TransactionHandle startNewTransaction() {
    std::lock_guard<std::mutex> lk(tx_meta_mutex);
...
std::optional<std::string> mvccReadRecord(const DataKey& key, TransactionHandle xid) {
    std::lock_guard<std::mutex> lk(heap_storage_mutex);
    TransactionHandle snap;
    {
        std::lock_guard<std::mutex> tlk(tx_meta_mutex);
...
void requestLock(const DataKey& key, TransactionHandle xid, LockAccess access) {
    ...
    std::unique_lock<std::mutex> ul(wl.queue_mutex);
    ...
    while (true) {
        ...
        if (!conflict) {
            ...
            {
                std::lock_guard<std::mutex> lk(lock_mgr_mutex);
