...
// ─────────────────────────────────────────────
enum class TokType {
    NUMBER, STRING, IDENT,          // operands
    OP,                             // > < >= <= = !=
    AND, OR, NOT,                   // logical
    SELECT, FROM, WHERE,            // SQL keywords
    STAR, COMMA,
    LPAREN, RPAREN,
    END
};
...
std::vector<Token> tokenize(const std::string& src) {
    ...
    while (i < src.size()) {
        ...
        if (src[i] == '\'') {
            ...
            tokens.push_back({TokType::STRING, src.substr(i + 1, j - i - 1)});
            i = j + 1; continue;
        }
        ...
        if (i + 1 < src.size()) {
            ...
            if (two == ">=" || two == "<=" || two == "!=") {
                tokens.push_back({TokType::OP, two}); i += 2; continue;
            }
        }

        // single-char operators / symbols
        if (src[i] == '>' || src[i] == '<' || src[i] == '=') {
            tokens.push_back({TokType::OP, {src[i]}}); i++; continue;
        }
        if (src[i] == '(') { tokens.push_back({TokType::LPAREN,  "("}); i++; continue; }
        if (src[i] == ')') { tokens.push_back({TokType::RPAREN,  ")"}); i++; continue; }
        if (src[i] == '*') { tokens.push_back({TokType::STAR,    "*"}); i++; continue; }
        if (src[i] == ',') { tokens.push_back({TokType::COMMA,   ","}); i++; continue; }

        // number
        if (isdigit(src[i]) || (src[i] == '-' && i + 1 < src.size() && isdigit(src[i+1]))) {
            size_t j = i + (src[i] == '-' ? 1 : 0);
            while (j < src.size() && (isdigit(src[j]) || src[j] == '.')) j++;
            tokens.push_back({TokType::NUMBER, src.substr(i, j - i)});
            i = j; continue;
        }
...
// Extract WHERE clause tokens (everything after WHERE keyword)
...
bool evalPostfix(const std::vector<Token>& rpn, const Row& row) {
    ...
    for (const auto& tok : rpn) {
        if (tok.type == TokType::NUMBER || tok.type == TokType::STRING) {
            ...
            if (it == row.end())
                throw std::runtime_error("unknown column: " + tok.val);
            st.push(it->second);
        }
            ...
            if (st.size() < 2) throw std::runtime_error("bad AND");
            std::string r = st.top(); st.pop();
            std::string l = st.top(); st.pop();
            st.push((l == "1" && r == "1") ? "1" : "0");
        }
        else if (tok.type == TokType::OR) {
            if (st.size() < 2) throw std::runtime_error("bad OR");
            std::string r = st.top(); st.pop();
            std::string l = st.top(); st.pop();
            st.push((l == "1" || r == "1") ? "1" : "0");
        }
...
std::vector<std::string> extractColumns(const std::vector<Token>& tokens) {
    ...
    while (i < tokens.size() &&
           tokens[i].type != TokType::FROM &&
           tokens[i].type != TokType::END) {
        if (tokens[i].type == TokType::IDENT)
            cols.push_back(tokens[i].val);
        i++;
    }
    return cols;
}
...
//  Print helpers
// ─────────────────────────────────────────────
void printRow(const Row& row, const std::vector<std::string>& cols, const std::vector<std::string>& allCols) {
...
int main() {
    ...
    // 4. Compound with parentheses — precedence override
