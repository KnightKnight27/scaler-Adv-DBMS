...
public:
    ...
    // Internal consistency checker — returns "" when healthy.
...
private:
    ...
    void release_all_locks(Txn& t) {
        ...
        t.locks.clear();
        // Anyone waiting on this txn can retry — drop the waits-for edges.
    ...
    // Oldest snapshot any active txn currently holds; defaults to clock_
    // when no txn is live (no future txn can see anything older).
