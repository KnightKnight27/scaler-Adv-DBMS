...
class BTree {
    ...
    void split_child(BNode* parent, int i) {
        ...
        // z gets the right half of y's keys; median is y->keys[T-1]
        ...
        // Promote median key to parent and link z as the new right child
    ...
    void delete_key(BNode* node, int key) {
        ...
        if (idx < (int)node->keys.size() && node->keys[idx] == key) {
            ...
            if ((int)node->children[idx]->keys.size() < T)
                fill(node, idx);
            // fill() may have merged children[idx] into children[idx-1]
