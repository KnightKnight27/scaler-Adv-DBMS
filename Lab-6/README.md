...
After splitChild(parent, idx, child):
...
```cpp
if (root is full) {
    new internal node = old root's only child
    splitChild(new root, 0, old root)
    root = new root
}
```
...
B-trees became the dominant on-disk index structure precisely because of properties 4 and the wide branching factor:
