// Lab 5: Dijkstra's Shunting-Yard expression evaluator + a minimal SQL SELECT
// parser/executor over an in-memory vector<Row>.
//   Build: g++ -std=c++17 -o sql_engine sql_engine.cpp && ./sql_engine
//
// Pipeline:  SQL string -> tokenize -> parse_select (AST) -> execute
//            WHERE clauses are evaluated by converting infix -> RPN (shunting-yard)
//            and running the RPN on a per-row variable map.
#include <iostream>
#include <sstream>
#include <stack>
#include <vector>
...
std::vector<std::string> tokenize(const std::string& expr) {
    ...
    while (i < n) {
        ...
        if (std::isdigit((unsigned char)expr[i]) ||
            ...
            if (i + 1 < n) {                      // two-char operators first
...
// ─────────────────── Part 2: minimal SELECT over vector<Row> ───────────────────
...
int main() {
    ...
    for (auto* sql : queries) {
        std::cout << "SQL: " << sql << "\n";
        auto res = execute(parse_select(sql), students);
        print_rows(res);
        std::cout << "\n";
    }
    return 0;
