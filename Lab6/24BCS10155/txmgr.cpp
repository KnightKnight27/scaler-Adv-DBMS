...
//
// Combines:
//  1. MVCC          — each write creates a new row version; readers see a
//                     consistent snapshot without blocking writers.
//  2. Strict 2PL    — locks acquired in the growing phase, all released at
//                     commit/abort (instantaneous shrinking phase).
//  3. Deadlock det. — waits-for graph cycle detection aborts a transaction.
//
...
void acquire_lock(const RowKey& key, TxID xid, LockMode mode) {
    ...
    for (auto& r : lq.requests) {                            // already hold it?
    ...
    while (true) {
        ...
        for (auto& r : lq.requests) {
            if (&r == &my_req) break;                        // only earlier requests
        ...
        if (!conflict) {
            my_req.granted = true;
            { std::lock_guard<std::mutex> lk(g_lm_mutex); g_waits_for.erase(xid); }
            return;
        }
