...
// Follows lab_sessions/lab_6.txt exactly.
//
//   MVCC      every write creates a new row version; readers see a consistent
//             snapshot (their txid) without blocking writers.
//   Strict 2PL  shared / exclusive row locks acquired during the GROWING
//             phase; all locks held until commit/abort (single shrinking
//             event), preventing cascading aborts.
//   Deadlock  waits-for graph + DFS cycle detection on each acquire;
//             the requester aborts (younger-victim policy).
...
struct Transaction {
    TxID     id;
    TxID     snapshot_xid;   // visibility horizon for this reader
...
struct RowVersion {
    std::string value;
    TxID        xmin;   // creator
    TxID        xmax;   // deleter / updater (0 == still live)
};
...
int main() {
    ...
    std::cout << "=== Scenario 1: MVCC Snapshot Isolation ===\n";
    {
        ...
        TxID t2 = tm.begin();   // t2 snapshot pins what's visible
        TxID t3 = tm.begin();
        ...
        print_val(v, t2, "balance");   // expects 1000 (t3 committed after t2 began)
        tm.commit(t2);
    }
