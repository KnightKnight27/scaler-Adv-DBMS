// ============================================================
//  Lab Session 4 (Part 2): Full B-Tree (minimum degree T)
//  Name      : Abdul Kalam Azad
//  Roll No.  : 24BCS10053
//  Build     : g++ -std=c++17 -o btree BTree.cpp && ./btree
//
//  Supports insert (split on the way down), search, and delete
//  (borrow from sibling / merge / replace with predecessor or
//  successor). This is the on-disk index structure used by
//  PostgreSQL, MySQL/InnoDB and SQLite.
// ============================================================

#include <iostream>
#include <vector>

const int T = 2;   // minimum degree; increase for higher fanout
...
class BTree {
    ...
    // Split child[i] of parent. child y must be full (2T-1 keys).
    // y keeps the left T-1 keys, z (new) gets the right T-1 keys,
    // and the median key is promoted into parent.
    void split_child(BNode* parent, int i) {
        BNode* y = parent->children[i];
        BNode* z = new BNode();
        z->leaf  = y->leaf;

        // Capture the median BEFORE shrinking y (the original brief
        // read y->keys[T-1] after resize -> out-of-bounds; fixed here).
        int med = y->keys[T - 1];

        // z takes the right half: keys [T .. 2T-2]
        z->keys.assign(y->keys.begin() + T, y->keys.end());
        if (!y->leaf) {
            z->children.assign(y->children.begin() + T, y->children.end());
            y->children.resize(T);
        }
        // y keeps the left half: keys [0 .. T-2]
        y->keys.resize(T - 1);
    ...
    // Largest key in the left subtree of keys[idx]
    ...
    // Smallest key in the right subtree of keys[idx]
    ...
    void delete_key(BNode* node, int key) {
        ...
        if (idx < (int)node->keys.size() && node->keys[idx] == key) {
            ...
            if (node->leaf) { std::cout << "Key not found: " << key << "\n"; return; }

            // 'last' must compare against the number of KEYS, not children
            // (the original brief compared with children.size() -> never
            // true -> could index past the last child after a merge).
            bool last = (idx == (int)node->keys.size());

            if ((int)node->children[idx]->keys.size() < T)
                fill(node, idx);

            // A merge at the end may have shrunk the key count, so the
            // target key now lives in the previous child.
    ...
    void destroy(BNode* node) {
        if (!node) return;
        if (!node->leaf)
            for (BNode* c : node->children) destroy(c);
        delete node;
    }
