...
// Compile: g++ -std=c++17 -o rbt rbt.cpp
// Run:     ./rbt
//
// Self-balancing BST — used in in-memory indexes (e.g. C++ std::map).
...
struct RBNode {
    ...
    explicit RBNode(int k) : key(k), color(RED) {}
};
...
class RedBlackTree {
    ...
    int black_height(RBNode* node) const {
        if (!node) return 0;
        int left  = black_height(node->left);
        int right = black_height(node->right);
        if (left != right) { std::cerr << "\nERROR: black-height mismatch!\n"; return -1; }
        return left + (node->color == BLACK ? 1 : 0);
    }

    void check_properties(RBNode* node) const {
        if (!node) return;
        // No two consecutive red nodes
        if (node->color == RED) {
            bool parent_red = node->parent && node->parent->color == RED;
            if (parent_red) std::cerr << "\nERROR: consecutive red nodes at " << node->key << "\n";
        }
        check_properties(node->left);
        check_properties(node->right);
    }
...
public:
    ...
    void remove(int key) {
        ...
        if (!z) { std::cout << "Key " << key << " not found\n"; return; }
...
int main() {
    ...
    // ── Stress test ──
    std::cout << "\n=== Stress: insert 1..100 then remove odd numbers ===\n";
    RedBlackTree rbt2;
    for (int i = 1; i <= 100; i++) rbt2.insert(i);
    for (int i = 1; i <= 100; i += 2) rbt2.remove(i);
    rbt2.print();  // should show even numbers in order
    rbt2.verify();
