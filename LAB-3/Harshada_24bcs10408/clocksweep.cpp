...
public:
    ...
    int fetch(int page_id) {
        ...
        if (it != page_to_frame.end()) {
            int idx = it->second;
            frames[idx].usage_count =
                std::min(frames[idx].usage_count + 1, 5);
        ...
        if (victim == -1) {
            std::cerr << "[ERROR] All frames are pinned. Cannot evict."
                      << std::endl;
            return -1;
        }
