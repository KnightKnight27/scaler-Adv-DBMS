...
OpMetadata getOpMetadata(const std::string& opSymbol) {
    if (opSymbol == "=" || opSymbol == "!=" ||
        opSymbol == "<" || opSymbol == "<=" ||
        opSymbol == ">" || opSymbol == ">=")   return {4, false, false};

    if (opSymbol == "NOT")                     return {3, true,  true };
    if (opSymbol == "AND")                     return {2, false, false};
    if (opSymbol == "OR")                      return {1, false, false};
...
int Relation::getColumnIndex(const std::string& columnName) const {
    for (std::size_t i = 0; i < headers.size(); ++i) {
        if (headers[i] == columnName) return static_cast<int>(i);
    }
    return -1;
}
...
std::vector<Token> tokenizeSQL(const std::string& rawSQL) {
    ...
    while (cursor < totalLen) {
        ...
        if (isValidAlpha(currentCh)) {
            ...
            if      (upperWord == "SELECT") outStream.push_back({LexemeType::KeywordSelect, upperWord, 0});
            else if (upperWord == "FROM")   outStream.push_back({LexemeType::KeywordFrom,   upperWord, 0});
            else if (upperWord == "WHERE")  outStream.push_back({LexemeType::KeywordWhere,  upperWord, 0});
            else if (upperWord == "ORDER")  outStream.push_back({LexemeType::KeywordOrder,  upperWord, 0});
            else if (upperWord == "BY")     outStream.push_back({LexemeType::KeywordBy,     upperWord, 0});
            else if (upperWord == "ASC")    outStream.push_back({LexemeType::KeywordAsc,    upperWord, 0});
            else if (upperWord == "DESC")   outStream.push_back({LexemeType::KeywordDesc,   upperWord, 0});
            else if (upperWord == "LIMIT")  outStream.push_back({LexemeType::KeywordLimit,  upperWord, 0});
        ...
        if (currentCh == '(') { outStream.push_back({LexemeType::ParenOpen, "("}); ++cursor; continue; }
        if (currentCh == ')') { outStream.push_back({LexemeType::ParenClose, ")"}); ++cursor; continue; }
        if (currentCh == ',') { outStream.push_back({LexemeType::CommaSeparator, ","}); ++cursor; continue; }
        if (currentCh == '*') { outStream.push_back({LexemeType::StarSymbol, "*"}); ++cursor; continue; }
        if (currentCh == '=') { outStream.push_back({LexemeType::LogicalOp, "="}); ++cursor; continue; }
...
std::vector<Token> shuntingYardRPN(const std::vector<Token>& infixTokens) {
    ...
    for (const auto& tok : infixTokens) {
        if (tok.type == LexemeType::TextIdentifier || 
            ...
            if (opStack.empty()) throw std::runtime_error("Unmatched ')' parenthesis.");
...
bool evaluateRPN(const std::vector<Token>& rpnSequence, const Relation& tableSchema, const Tuple& rowInstance) {
    ...
    for (const auto& tok : rpnSequence) {
        ...
        if (meta.is_unary_op) {
            if (evalStack.empty()) throw std::runtime_error("Unary NOT missing operand.");
        ...
        if (evalStack.size() < 2) throw std::runtime_error("Binary operator missing operands: " + tok.raw_string);

        CellValue rightOperand = evalStack.top(); evalStack.pop();
        CellValue leftOperand = evalStack.top(); evalStack.pop();
        ...
        if (matchType) {
            if      (tok.raw_string == "=")  conditionMet = (compareRes == 0);
            else if (tok.raw_string == "!=") conditionMet = (compareRes != 0);
            else if (tok.raw_string == "<")  conditionMet = (compareRes < 0);
            else if (tok.raw_string == "<=") conditionMet = (compareRes <= 0);
            else if (tok.raw_string == ">")  conditionMet = (compareRes > 0);
            else if (tok.raw_string == ">=") conditionMet = (compareRes >= 0);
    ...
    if (evalStack.empty()) return true; // Empty WHERE clause is always true
...
std::string stringifyRPN(const std::vector<Token>& rpnSequence) {
    ...
    for (const auto& tok : rpnSequence) {
        if (!flatString.empty()) flatString += " ";
...
ParsedQuery compileQuery(const std::string& sqlStatement) {
    ...
    auto verifyNext = [&](LexemeType expectedType, const std::string& err) {
        if (tokenStream[idx].type != expectedType) throw std::runtime_error("Parsing failure. Expected: " + err);
    };
...
Relation executeQuery(const ParsedQuery& plan, const Relation& sourceDatabase) {
    ...
    for (const auto& col : targetHeaders) {
        ...
        if (cIdx < 0) throw std::runtime_error("Invalid column requested: " + col);
    ...
    if (!plan.order_column.empty()) {
        ...
        if (sortIdx < 0) throw std::runtime_error("ORDER BY column not found in result output: " + plan.order_column);
...
void printTable(const Relation& table, std::ostream& outputStream) {
    ...
    auto stringifyCell = [](const CellValue& cv) -> std::string {
        if (std::holds_alternative<long long>(cv)) return std::to_string(std::get<long long>(cv));
