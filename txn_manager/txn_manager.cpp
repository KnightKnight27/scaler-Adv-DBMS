// Lab 6: Transaction manager combining MVCC + Strict 2PL + deadlock detection.
//   Build: g++ -std=c++17 -pthread -o txmgr txn_manager.cpp && ./txmgr
//
//   * MVCC      - each write appends a new row version; readers use a snapshot
//                 and never block writers.
//   * Strict 2PL- locks are acquired in the growing phase and all released at
//                 commit/abort (the shrinking phase is instantaneous at the end).
//   * Deadlock  - a waits-for graph is checked for cycles on every wait; a cycle
//                 throws DeadlockException so the caller can abort.
#include <iostream>
...
struct Transaction {
    TxID     id;
    TxID     snapshot_xid;            // sees commits with xid < snapshot_xid
...
void acquire_lock(const RowKey& key, TxID xid, LockMode mode) {
    ...
    while (true) {
        ...
        for (auto& r : lq.requests) {
            if (&r == &my_req) break;             // only earlier requests block us
...
int main() {
    ...
    std::cout << "=== Scenario 1: MVCC Snapshot Isolation ===\n";
    {
        ...
        print_val(tm.read(t2, "balance"), t2, "balance");   // expect 1000
        tm.commit(t2);
    }
