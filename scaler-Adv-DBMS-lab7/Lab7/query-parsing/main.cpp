...
// Query Parsing: recursive-descent SQL parser in C++17.
// Tokenizes a SQL string, builds an AST, and evaluates it against an
// in-memory row set.
    ...
    vector<Lexeme> scan() {
        ...
        while (cursor < (int)source.size()) {
            ...
            if (isalpha(source[cursor])) {
                ...
                std::string digits;
                while (cursor < (int)source.size() && isdigit(source[cursor]))
...
int main() {
    ...
    staff.push_back({"Rajveer", 1, 22});
    staff.push_back({"Ananya",  2, 22});
