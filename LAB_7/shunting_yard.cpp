...
std::vector<std::string> tokenize(const std::string &expr)
{
    ...
    while (i < n)
    {
        ...
        if (std::isdigit(static_cast<unsigned char>(expr[i])) || (expr[i] == '.' && i + 1 < n && std::isdigit(static_cast<unsigned char>(expr[i + 1]))))
        {
            int j = i;
            while (j < n && (std::isdigit(static_cast<unsigned char>(expr[j])) || expr[j] == '.'))
                j++;
