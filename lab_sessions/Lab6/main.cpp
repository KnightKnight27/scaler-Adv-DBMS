...
int main() {
    ...
    std::cout << "\n=== Scenario 4: Deadlock Detection ===\n";
    {
        ...
        tm.locks().acquire("A", t8, LockMode::EXCLUSIVE);
        tm.locks().acquire("B", t9, LockMode::EXCLUSIVE);

        std::thread th1([&]() {
            try {
                tm.locks().acquire("B", t8, LockMode::EXCLUSIVE);
                tm.commit(t8);
        ...
        try {
            tm.locks().acquire("A", t9, LockMode::EXCLUSIVE);
            tm.commit(t9);
        } catch (DeadlockException& e) {
            std::cout << "  " << e.what() << "\n";
            tm.abort(t9);
