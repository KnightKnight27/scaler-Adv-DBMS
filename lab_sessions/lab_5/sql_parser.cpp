...
SelectQuery parse_select(const std::string& sql) {
    ...
    while (ss >> word && to_upper(word) != "FROM") {
        ...
        if (word != "*") q.columns.push_back(word);
    }
    ss >> q.from;
    ...
    while (ss >> word) {
        ...
        if (kw == "WHERE") {
            ...
            while (ss >> w2) {
                std::string u = to_upper(w2);
                if (u == "ORDER" || u == "LIMIT") { word = w2; goto next_clause; }
                clause += (clause.empty() ? "" : " ") + w2;
            }
            q.where_raw = clause; break;
        next_clause:
            q.where_raw = clause;
            kw = to_upper(word);
        }
...
void print_rows(const std::vector<Row>& rows, const std::vector<std::string>& cols) {
    for (const auto& row : rows) {
        auto keys = cols.empty() ? [&](){
            std::vector<std::string> ks;
            for (auto& [k,v] : row.cols) ks.push_back(k);
        ...
        for (const auto& k : keys) {
            ...
            std::cout << k << "=";
            if (auto* d = std::get_if<double>(&it->second)) std::cout << (int)*d;
...
int main() {
    // ── Part 1: Shunting-Yard demo ────────────────────────────────────────────
    std::string expr = "age * 2 + gpa / 1.0 > 45";
    ...
    std::unordered_map<std::string, double> vars = {{"age", 22}, {"gpa", 3.8}};
    std::cout << "Eval (age=22, gpa=3.8): " << (eval_rpn(rpn, vars) ? "true" : "false") << "\n\n";
