...
// AST node: either a comparison leaf (col op val) or a logical node (AND/OR)
struct Node {
...
std::vector<std::string> tokenize(const std::string& q) {
    ...
    while (i < n) {
        ...
        if (std::isalpha(c) || c == '_') {
            ...
            tokens.push_back({q[i], q[i+1]}); i += 2;
        } else {
...
public:
    ...
    ParsedQuery parseQuery() {
        ...
        if (cursor_ != tokens_.size())
            throw std::runtime_error("Unexpected token after WHERE: '" + tokens_[cursor_] + "'");
...
private:
    ...
    void expectKeyword(const std::string& kw) {
        ...
        if (got != kw) throw std::runtime_error("Expected " + kw + ", got '" + got + "'");
    }
...
bool evaluate(const Node* node, const Employee& e) {
    ...
    else throw std::runtime_error("Unknown column: '" + node->col + "'");
