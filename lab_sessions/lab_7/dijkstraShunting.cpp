...
// SQL evaluation order: comparisons bind tighter than AND, AND tighter than OR
...
std::vector<std::string> tokenize(const std::string& query) {
    ...
    while (i < n) {
        ...
        if (std::isalpha(c) || c == '_') {
            ...
            tokens.push_back((up == "AND" || up == "OR") ? up : lex);
            continue;
        }
        ...
        if ((query[i] == '>' || query[i] == '<' || query[i] == '!') && i+1 < n && query[i+1] == '=') {
            tokens.push_back({query[i], query[i+1]});
            i += 2; continue;
        }

        if (query[i] == '&' && i+1 < n && query[i+1] == '&') { tokens.push_back("AND"); i += 2; continue; }
        if (query[i] == '|' && i+1 < n && query[i+1] == '|') { tokens.push_back("OR");  i += 2; continue; }
    ...
    for (const auto& tok : tokens) {
        if (tok == "(") {
            ...
            while (!ops.empty() && ops.top() != "(") {
                ...
                if (topPrec > info.precedence || (topPrec == info.precedence && !info.right_assoc))
...
int resolveField(const std::string& field, const Employee& e) {
    ...
    throw std::runtime_error("Unknown column: '" + field + "'");
}
...
bool evaluatePostfix(const std::vector<std::string>& postfix, const Employee& e) {
    ...
    if (st.size() != 1)
        throw std::runtime_error("Malformed expression: stack does not collapse to one value");
    return st.top() != 0;
}
...
int main() {
    try {
        ...
        const std::vector<Employee> employees = {
            {"Rama Krishnan", 1, 19}, {"Aarav",   2, 20},
            {"Karan",         3, 19}, {"Sneha",   4, 21},
            {"Vivaan",        5, 20}, {"Ishaan",  6, 31},
