// ============================================================
//  Lab 5: AST-based SQL SELECT Parser (recursive descent)
//  Name      : Abdul Kalam Azad
//  Roll No.  : 24BCS10053
//  Build     : g++ -std=c++17 -Wall -Wextra main.cpp -o query_parser && ./query_parser
// ============================================================
 ...
 *
 * This program tokenizes a SELECT query, parses it into an AST (Abstract Syntax Tree)
 * with robust memory safety (std::unique_ptr), and evaluates the condition against
...
struct Parser {
    ...
    /**
     * @brief Consumes and returns the current token, advancing the parser position.
     */
    std::string eat() {
    ...
    /**
     * @brief Parses an expression with potential parenthesis and logical operators (OR).
     */
    std::unique_ptr<Node> parseExpression() {
        ...
        while (position < tokens.size() && toUppercase(tokens[position].text) == "OR") {
            eat(); // eat "OR"
...
bool evaluate(const Node* node, const Employee& employee) {
    ...
    if (node->op == "OR") {
        return evaluate(node->l.get(), employee) || evaluate(node->r.get(), employee);
    }
    ...
    if (node->op == ">") return fieldValue > node->val;
    if (node->op == "<") return fieldValue < node->val;
    if (node->op == ">=") return fieldValue >= node->val;
    if (node->op == "<=") return fieldValue <= node->val;
    if (node->op == "=") return fieldValue == node->val;
...
int main() {
    try {
        ...
        std::string queryText = "SELECT name FROM employees WHERE id >= 3 OR age < 20";
