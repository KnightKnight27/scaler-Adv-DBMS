...
Shared Buffer Array (circular, simplified):
  ...
  uc=2     uc=0     uc=1     uc=0     uc=3     uc=2
  pin=0    pin=0    pin=1    pin=0    pin=0    pin=0
                     ^
                     sweep hand

Hand at Buf 2: pin=1 → skip.
...
+-------------------+
| PageHeaderData    |  ← pd_lsn, pd_checksum, pd_lower, pd_upper, pd_special
+-------------------+
| ItemIdData[0]     |
| ItemIdData[1]     |  ← line pointers (grow downward)
| ...               |
+-------------------+  ← pd_lower
|                   |
|    Free Space     |
...
+-------------------+  ← pd_upper
| Index Tuple N     |
| Index Tuple 1     |  ← actual tuples (grow upward)
| Index Tuple 0     |
+-------------------+
| Special Space     |  ← high key on leaf pages
+-------------------+
```
...
+-----------------------------+
| xl_tot_len, xl_xid          |
| xl_prev (back-pointer)      |
| xl_rmid (resource manager)  |
| xl_info (flags + subtype)   |
| xl_crc (checksum)           |
+-----------------------------+
| Record Data:                |
|   Block headers (rel/db/    |
|     forknum, block number)  |
|   Payload (new page image   |
|     or delta)               |
+-----------------------------+
```
