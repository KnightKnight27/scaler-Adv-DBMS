...
> InnoDB is MySQL's default storage engine, and it makes a set of choices that
> are the deliberate *mirror image* of PostgreSQL's: the table **is** a clustered
> B+tree (not a heap), rows are updated **in place** (not appended), old versions
> live in a separate **undo log** (not in the table), and crash recovery does a
> full **redo + undo** pass (not redo-only). This document explains those choices
> and *why* they were made. I ground it in the B-tree I built in **Lab 6**, the
> buffer pool from **Lab 3**, the locking + deadlock detector from **Lab 8**, the
> real InnoDB numbers from my **Lab 2** benchmark, and the **ARIES** recovery
> paper (InnoDB's redo log is a direct ARIES descendant).
...
Since MySQL 5.5 it is the default engine. Its design target is **high-concurrency
OLTP** — many short transactions doing point lookups and small updates by primary
key — which is exactly the workload its clustered-index + in-place-update design
optimizes for.
...
Lab 3 implemented **clock-sweep** with a `usage_count` (PostgreSQL's policy);
InnoDB instead uses an **LRU list with midpoint insertion**, and comparing the
two is illuminating because they solve the *same* eviction problem differently:
...
This is one of the suggested questions, and the clean answer is that the two logs
protect **opposite directions in time**:
...
InnoDB's redo log is a textbook **ARIES** implementation (Mohan et al.): every
change gets a **Log Sequence Number (LSN)**, each page stores the LSN of its last
change, updates are applied **in place** (not shadow-paged), and InnoDB takes
**fuzzy checkpoints** so it never has to stop the world to flush. Crash recovery
runs ARIES's passes:
...
> storage decisions — and every downstream difference follows from that one split.
...
2. **Redo and undo are orthogonal and you need both.** Redo protects committed
   work *forward* in time (durability); undo removes uncommitted work *backward*
   (atomicity) and reconstructs old MVCC reads. This finally made the suggested
   "why both logs?" question obvious.

3. **In-place updates are *why* InnoDB must run ARIES's undo pass.** Reading the
   ARIES paper next to my Topic 2 work crystallized it: PostgreSQL skips undo
