...
```mermaid
graph TD
    MT["MemTable<br>(mutable, in memory - skip list)"]
    IMT["Immutable MemTable<br>(frozen, waiting for flush)"]
    L0["L0 SSTables<br>(overlapping key ranges)"]
    L1["L1 SSTables<br>(sorted, non-overlapping)"]
    L2["L2 SSTables<br>(sorted, non-overlapping)"]
    Ln["Ln SSTables<br>(largest, oldest data)"]
