...
```mermaid
graph TD
    Root["10 | 30<br>(Internal Node - keys only)"]
    L1["1 | 5<br>(Internal Node)"]
    R1["15 | 20<br>(Internal Node)"]
    Leaf["Leaf Page (Full Row Data)<br>• pk=1, name='Alice', email='a@x'...<br>• pk=5, name='Bob', email='b@x'..."]
...
```mermaid
graph TD
    Root["'alice@x' | 'bob@x'<br>(Secondary Index Node)"]
...
```mermaid
graph TD
    Buffer["Redo Log Buffer<br>(In-memory - threads append here)"]
