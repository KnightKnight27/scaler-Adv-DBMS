...
### 3.6 Type System: Dynamic vs Static

A subtle but architecturally important divergence.

- **SQLite — dynamic typing with "type affinity":** A column's declared type is only a *hint*. SQLite stores each value with its own type (INTEGER, REAL, TEXT, BLOB, NULL) and applies affinity rules to coerce on insert. You can store a string in an `INTEGER` column. This flexibility minimizes schema-migration friction — useful when an app's schema evolves rapidly across mobile releases — but pushes type-correctness into the application.
- **PostgreSQL — strict static typing:** Every column has a rigid type checked at write time; a type mismatch is an error. This catches bugs early, enables a rich type system (arrays, JSONB, ranges, user-defined types, enums), and gives the optimizer precise per-type statistics and operator classes for indexing.

The trade-off mirrors the whole comparison: SQLite optimizes for *flexibility and low friction in a single embedded app*; PostgreSQL optimizes for *correctness and rich querying in a shared, long-lived server*.
...
- **In-process library** — no daemon, socket, or background service to drain battery/memory or crash independently of the app.
- **Single file** — trivially copied, backed up, and bundled; fits the sandboxed per-app storage of Android/iOS exactly.
- **Single-writer pattern is a non-issue** — a phone app is effectively one writer, so SQLite's one-writer-at-a-time ceiling never bites.
- **Tiny footprint (~600 KB) + atomic single-file commits** survive abrupt power loss or the OS killing the app mid-write — precisely the failure modes mobile faces.

**Q: Why is PostgreSQL preferred for large multi-user systems?**
- **MVCC** lets thousands of clients read and write concurrently without serializing on a global lock — readers don't block writers.
- **Process-per-connection + shared buffers** exploit many CPU cores and isolate failures (one crashed backend doesn't kill others).
- **Server-grade features** — streaming/logical replication, parallel query, a cost-based optimizer, rich index types (GIN/GiST/BRIN) — handle the complex queries, scale, and high-availability that server workloads demand.

**Q: What architectural decisions lead to these differences?**
The **library-vs-server** choice is the root cause. Embedding forces SQLite toward OS file locking (simple, single-writer) and one self-contained file. The server model lets PostgreSQL run persistent background workers (autovacuum, walwriter, checkpointer), keep shared memory across backends, and implement MVCC + replication. Every other difference — concurrency model, storage layout, type strictness — flows from that single decision.
