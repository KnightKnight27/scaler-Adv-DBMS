...
**ClockSweep replacement policy:**PostgreSQL does not use LRU. Instead it uses ClockSweep (implemented in `freelist.c`). Each buffer frame has a `usage_count` (0–5). A circular "clock hand" sweeps through frames:
...
**Visibility rule:**A tuple version is visible to transaction T (with snapshot S) if:
