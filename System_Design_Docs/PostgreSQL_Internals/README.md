...
PostgreSQL descends from the **POSTGRES** project at UC Berkeley (Michael
Stonebraker, 1986), which set out to build a database that was *correct under
heavy concurrency* and *extensible* (custom types, operators, index methods).
SQL support arrived in 1994–96, and the open-source PostgreSQL we use today
inherited three non-negotiable goals from that lineage:

1. **Many users at once, without corrupting each other.** This is the root
   requirement that forces MVCC and a shared buffer pool.
2. **A committed transaction is never lost, even on a crash mid-write.** This
   forces write-ahead logging.
3. **Run arbitrary queries efficiently.** This forces a cost-based optimizer
   that reasons about statistics rather than executing SQL literally.

Everything in this document is downstream of those three goals. I covered the
*external* contrast with SQLite in my `PostgreSQL_vs_SQLite` submission; here the
focus is the *internal machinery* a single backend uses to honor them.

### The one-backend-per-connection model

A `postmaster` daemon listens on a socket and **forks a dedicated backend
process** for each client connection. All backends share one block of memory
(`shared_buffers`) plus background helpers (WAL writer, checkpointer, autovacuum,
background writer). This process model — described in Hellerstein, Stonebraker &
Hamilton's *Architecture of a Database System* as the "process-per-connection"
design — is the frame for everything below: the buffer pool must be **shared and
concurrency-safe** precisely because many backends hit it simultaneously.
...
When `shared_buffers` is full, PostgreSQL picks a victim with the **clock-sweep**
algorithm — and this is precisely the data structure I implemented in Lab 3:
...
My Lab 3 notes also list what a teaching version omits, and those omissions *are*
the real buffer manager:
...
**An insightful contrast I only understood after reading ARIES:** ARIES has
...
The structure below is the plan PostgreSQL's cost-based planner chooses for Q2;
the **71.9 ms** end-to-end is my measured Lab 2 result. Per-node figures are
illustrative of how PostgreSQL annotates a plan and of the *estimated-vs-actual*
point — not separately captured timings.
...
4. **PostgreSQL recovery is redo-only because MVCC already handles undo.**
   Reading ARIES made this click: PostgreSQL takes ARIES's redo + repeating-history
   + LSN-in-page, but skips the physical undo pass — aborted writes are simply
   *never visible* and get vacuumed. That's a genuinely different trade-off from
   InnoDB, which keeps explicit undo logs.
