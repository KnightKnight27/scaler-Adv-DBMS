...
> **Course:** Advanced DBMS | **Name:** T. Abdul Kalam Azad | **Roll Number:** 24BCS10053
...
#### HOT (Heap-Only Tuples) — Mitigating Append-Only Update Cost

Append-only updates have a hidden tax: a new tuple version normally needs **new index entries in every index**, even if the update didn't touch any indexed column. This bloats indexes and amplifies writes. PostgreSQL's **HOT** optimization avoids this when two conditions hold: (1) the UPDATE changes no indexed column, and (2) the new version fits on the **same heap page**.

```
HOT chain on a single heap page (UPDATE that changed only a non-indexed column):
...
The index keeps pointing at the original "root" tuple; new versions are reachable only by following the in-page `t_ctid` chain. No index write happens at all. Better still, dead HOT tuples can be reclaimed by a lightweight **single-page prune** during normal page access — without waiting for a full VACUUM. HOT is a major reason PostgreSQL's append-only model is viable for UPDATE-heavy OLTP; it is also why a low `fillfactor` (leaving free space on each page for in-place HOT updates) is a classic tuning lever.
...
**Q: How do pages move through the buffer manager?**
A backend computes a buffer tag `{RelFileNode, ForkNumber, BlockNumber}` and probes the shared hash table. On a hit it pins the buffer and reads it; on a miss the clock-sweep picks a victim (writing it out first if dirty, after its WAL is flushed — the write-ahead rule), reads the page from disk into that slot, and updates the table. Dirty pages are normally written back by the **bgwriter** (proactively) and the **checkpointer** (at checkpoints), so foreground queries rarely block on I/O. Sequential scans use a small ring buffer so they don't evict the whole cache.

**Q: How does PostgreSQL implement MVCC?**
Every tuple carries `t_xmin`/`t_xmax`. INSERT sets xmin; UPDATE writes a *new* tuple and stamps the old one's xmax (never in-place); DELETE just sets xmax. A transaction's **snapshot** `{xmin, xmax, xip}` decides visibility, so readers see a consistent point-in-time view without taking locks — readers never block writers and vice-versa. The cost is dead tuples.

**Q: Why is VACUUM necessary?**
Because MVCC leaves dead tuple versions behind. VACUUM reclaims their space (via the Free Space Map), removes their index entries, maintains the visibility map (enabling index-only scans), and — critically — **freezes** old XIDs to prevent transaction-ID wraparound. Without it, tables bloat and eventually become unreadable.

**Q: How does WAL guarantee durability?**
The write-ahead rule: a change's WAL record must reach disk *before* the modified data page is written, and a COMMIT returns success only *after* its WAL record is fsynced. On crash, replay from the last checkpoint reconstructs every committed-but-unflushed change; full-page images guard against torn writes. Because WAL is written sequentially, this is fast.

**Q: How does query planning rely on collected statistics?**
The cost-based planner estimates row counts and selectivity from `pg_statistic` (n_distinct, most-common-values, histograms) populated by ANALYZE, then picks join order/method (Hash vs Nested Loop vs Merge) and scan type. Stale stats produce wrong cardinality estimates, which cascade into bad plan choices (see Experiment 5) — accurate statistics are the planner's only view of the data.
