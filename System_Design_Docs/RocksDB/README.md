...
> A study of RocksDB: Facebook's embeddable **LSM-tree** key-value store: covering the MemTable → SSTable write path, the layered read path, Bloom filters, and compaction. The amplification numbers below come from a **custom C++ benchmark I wrote and compiled against `librocksdb` 11.1.1** and ran locally; results are quoted inline.
...
1. **LSM inverts the B-tree trade-off.** By writing sequentially and merging later, RocksDB makes writes cheap (417 K ops/s ingest) and pushes the cost onto background compaction and multi-level reads. That is why it is chosen for write-heavy/flash workloads.
