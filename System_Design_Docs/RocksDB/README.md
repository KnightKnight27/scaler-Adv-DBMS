...
> building a B-tree-based engine myself (MiniDB), studying RocksDB felt like
> learning a second, totally different philosophy of how a database can work.
...
> **Contrast with MiniDB:** in my B-tree engine, an insert finds the right leaf
...
> **From building MiniDB:** my B-tree `get` is one downward traversal to a single
...
MiniDB. The structures above the WAL differ wildly (heap, clustered B+ tree, LSM),
...
- My capstone implementation, MiniDB (`MiniDB_Projects/Team_HarshDB/`) - a B-tree/heap engine, used here as the in-place-update counterpoint to RocksDB's append-only LSM design
