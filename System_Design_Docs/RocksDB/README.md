...
> data in **B-trees** updated **in place**. RocksDB is the deliberate opposite: a
> **Log-Structured Merge-tree (LSM)** that *never* updates in place — it batches
> writes in memory and flushes them as large sequential files, then merges those
> files in the background. This document explains that design and, above all,
> *why* it is write-optimized. RocksDB descends directly from Google's
> **Bigtable** (Bigtable → LevelDB → RocksDB), so I ground the internals in the
> Bigtable paper from my course resources, use the Red-Black tree from **Lab 5**
> as the in-memory MemTable analogue, and use the B-tree from **Lab 6** as the
> in-place baseline that makes the LSM trade-off concrete.
>
> *Honesty note:* I did not run `db_bench` myself, so the amplification figures in
> §5 are **derived from the RocksDB documentation and the LSM literature (cited)**
> and from my own analysis using Lab 6 — they are clearly labelled as such, not
> presented as my measurements.
...
and if the page is full, a split that triggers *more* random writes. For a
**write-heavy** workload (logging, time-series, metrics, message queues, write
paths of large web services), that storm of small random writes is the
bottleneck — random I/O is the slowest thing a disk does, and on SSDs it also
causes device-level write amplification and wear.
...
strategy is choosing where on the amplification trade-off curve to sit.**
...
> Athanassoulis et al.'s **RUM conjecture** states you can optimize at most two of
> **R**ead, **U**pdate, and **M**emory (space) — improving one usually worsens
> another. An LSM consciously sacrifices **read** and **space** amplification to
> win **update** (write) performance. A B-tree makes the opposite choice. There is
> no free lunch; there is only "which amplification can your workload afford?"
...
amplification in the **~10–30×** range — consistent with this back-of-envelope
bound. **Universal/tiered** compaction rewrites data far fewer times, so its WA
