...
- **Path resolution** — kernel walks the directory tree through the dcache (directory entry cache). On a miss, it reads directory blocks from disk.
- **Inode lookup** — finds the inode: stores permissions, size, timestamps, and pointers to data blocks.
- **Permission check** — compares file's UID/GID/mode against the calling process's credentials.
- **File description** — kernel allocates a `struct file` in kernel memory and a file descriptor (`fd`) — the smallest available non-negative integer in the process's fd table.
- **Returns fd** — back to user-space (e.g. 3).

### 2. `read(fd, buf, count)` — Read bytes
- **Mode switch** — SYSCALL instruction traps into ring 0 (kernel mode).
- **File position** — kernel reads the current offset (starts at 0 after open).
- **Page cache check**:
...
- **Advance offset** — file position += bytes actually read.
- **Returns** bytes read, 0 on EOF, -1 on error.

### 3. `close(fd)` — Close input
- Decrements reference count on the `struct file`.
- At zero: frees the file description, releases the fd number.
- Does not flush dirty data — page cache writeback is asynchronous.
...
- **Mode switch** — SYSCALL instruction into ring 0.
- **Copy to page cache** — kernel copies data from user buffer → kernel page (now marked dirty).
- **Inode update** — file size and modification time updated in memory.
- **Returns immediately** — data is NOT on disk yet. This is the write-back model.
- **Eventual flush** — the kernel's flusher thread writes dirty pages to disk based on `vm.dirty_expire_centisecs` (default ~30 s) or when memory pressure hits.
- *Need durability guarantees?* Call `fsync(fd)` before `close()`.

### 6. `close(fd)` — Close output
- Releases the fd.
- May flush metadata (size, timestamps) to the filesystem journal.
- Dirty page data still flushes asynchronously unless fsync was called.
