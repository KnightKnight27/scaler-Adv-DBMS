    ...
    void fix_delete(RBNode* x, RBNode* x_parent) {
        while (x != root && (!x || x->color == BLACK)) {
            if (x == (x_parent ? x_parent->left : nullptr)) {
                ...
                if (w && w->color == RED) {                 // Case 1: sibling is red
                    w->color        = BLACK;
                    x_parent->color = RED;
                ...
                if ((!w->left  || w->left->color  == BLACK) &&
                    (!w->right || w->right->color == BLACK)) { // Case 2: sibling's children black
                    if (w) w->color = RED;
                    x = x_parent; x_parent = x->parent;
                } else {
                    if (!w->right || w->right->color == BLACK) { // Case 3: sibling's right black
                        if (w->left) w->left->color = BLACK;
                        w->color = RED;
                        right_rotate(w);
                        w = x_parent->right;
                    }
                    w->color        = x_parent->color;      // Case 4: sibling's right red
