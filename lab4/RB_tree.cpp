...
public:
    ...
    // Returns the node with val == value, or nullptr.
    ...
    // Prints the tree sideways; rightmost subtree appears at the top.
    void printPretty(std::ostream& os) const { printPretty(os, root_, 0); }

    // Validates the four RB invariants: root is black, no red-red edge,
    // every leaf path has equal black-height, and the BST order holds.
    bool validate() const {
        if (!root_) return true;
        if (root_->color != Color::Black) return false;
        return blackHeight(root_) != -1 && isBst(root_, nullptr, nullptr);
    }
...
private:
        ...
        while (z->parent && z->parent->color == Color::Red) {
            ...
            if (parent == grandparent->left) {
                ...
                if (uncle && uncle->color == Color::Red) {
                    ...
                    if (z == parent->right) {
                        // Case 2: zig-zag → rotate to a straight line.
                        z = parent;
                        leftRotate(z);
                        parent = z->parent;
                    }
    ...
    // Returns the uniform black-height (counting the NIL sentinel as black)
    // or -1 if the subtree violates the no-red-red or equal-black-height rule.
