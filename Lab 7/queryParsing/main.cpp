...
bool applyFilter(Expression *expr, const Employee &row)
{
  ...
  if (!bin)
    throw runtime_error("expression not valid");
  ...
  for (const auto &row : employees)
  {
    if (applyFilter(statement.whereFilter, row))
    {
      if (statement.column == "name")
        cout << row.getString("name") << endl;
      else if (statement.column == "id")
        cout << row.getInt("id") << endl;
      else if (statement.column == "age")
        cout << row.getInt("age") << endl;
    }
  }
}
