// Lab 4, Part 2: full B-Tree of minimum degree T (the on-disk index shape used
// by PostgreSQL / MySQL / SQLite). Supports insert (split on the way down),
// search, and delete (borrow / merge on the way down).
//   Build: g++ -std=c++17 -o btree btree.cpp && ./btree
//
// Every node holds between T-1 and 2T-1 keys (root may hold as few as 1),
// and an internal node with k keys has k+1 children.
#include <iostream>
#include <vector>
...
class BTree {
    ...
    // Split parent->children[i] (which must be full: 2T-1 keys) around its median.
    ...
    int get_predecessor(BNode* node, int idx) {        // max key of left subtree
        BNode* cur = node->children[idx];
        while (!cur->leaf) cur = cur->children.back();
        return cur->keys.back();
    }

    int get_successor(BNode* node, int idx) {          // min key of right subtree
    ...
    void delete_key(BNode* node, int key) {
        ...
        if (idx < (int)node->keys.size() && node->keys[idx] == key) {
            ...
            if ((int)node->children[idx]->keys.size() < T)
                fill(node, idx);
            // fill() may have merged children[idx] into its left sibling.
