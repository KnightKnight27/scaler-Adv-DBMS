  ...
  if (!select->hasJoin) {
    ...
    if (select->hasWhere) {
      ...
      if (canUseIndex && indexScanCost < tableScanCost) {
        ...
        return std::make_unique<IndexScanNode>(table, select->whereVal);
      } else {
        ...
        return std::make_unique<FilterNode>(std::move(scan), select->whereCol,
                                            select->whereOp, select->whereVal);
      }
    } else {
...
void Optimizer::executeShowTables() {
    std::cout << "Tables:\n";
...
void Optimizer::executeShowSchema(ShowSchemaAST* ast) {
    if (!catalog_.hasTable(ast->tableName)) {
        std::cout << "Table '" << ast->tableName << "' not found.\n";
        return;
    }
    Table* table = catalog_.getTable(ast->tableName);
    const Schema& schema = table->getSchema();
    std::cout << "Schema for table " << ast->tableName << ":\n";
