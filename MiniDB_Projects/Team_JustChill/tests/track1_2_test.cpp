...
static void test_heap_file_basics() {
    ...
    // Reopen and read to verify persistence
    {
        ...
        Page page0;
        hf.readPage(0, &page0);
        CHECK(std::strcmp(page0.data, "Data on page 0") == 0);

        Page page1;
        hf.readPage(1, &page1);
        CHECK(std::strcmp(page1.data, "Data on page 1") == 0);
    }

    fs::remove(filename);
    std::cout << "[ok] HeapFile: basics\n";
}

static void test_buffer_pool_basics() {
...

    {
        ...
        CHECK(pg0_again != nullptr);
        CHECK(std::strcmp(pg0_again->data, "Page 0 Content") == 0);
        ...
        CHECK(pg1_again != nullptr);
        CHECK(std::strcmp(pg1_again->data, "Page 1 Content") == 0);
        bp.unpinPage(p1, false);
    }

    fs::remove(filename);
    std::cout << "[ok] BufferPool: basics & LRU eviction\n";
}

static void test_buffer_pool_concurrency() {
    ...
    fs::remove(filename);
    std::cout << "[ok] BufferPool: concurrency\n";
}

static std::mutex mock_db_lock;
static std::vector<std::string> received_queries;

static void mock_execute_sql_callback(const std::string& sql) {
    std::lock_guard<std::mutex> lock(mock_db_lock);
    received_queries.push_back(sql);
}

static void test_wal_basics() {
    ...
    CHECK(lines.size() == 2);
    CHECK(lines[0] == "INSERT INTO students VALUES (1, 'Alice');");
    CHECK(lines[1] == "DELETE FROM students WHERE id = 1;");

    fs::remove(filename);
    std::cout << "[ok] WAL: serialization & persistence\n";
}

static void test_replication_basics() {
    ...
    // Verify replication
    std::lock_guard<std::mutex> lock(mock_db_lock);
    CHECK(received_queries.size() == 2);
    CHECK(received_queries[0] == "INSERT INTO t VALUES (1, 'Bob')");
    CHECK(received_queries[1] == "DELETE FROM t WHERE id = 1");

    std::cout << "[ok] Replication: statement transfer & replica callback\n";
}

// Dummy hanging server function
static void run_hanging_replica_server(int port, std::atomic<bool>& stop_flag) {
...
static void test_replication_failure_scenarios() {
    ...
    CHECK(ok_hanging == false);
    CHECK(diff.count() >= 1.5 && diff.count() < 2.5);
