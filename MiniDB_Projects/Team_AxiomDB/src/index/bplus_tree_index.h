...
// ===========================================================================
// BPlusTreeIndex -- a disk-resident B+tree mapping byte-string keys to RIDs.
//
...
// scan.  It also backs the KV ClassicStorageEngine's ordered get/scan.
//
...
class BPlusTreeIndex {
 public:
  ...
  BPlusTreeIndex(BufferPoolManager* bpm, page_id_t meta_page_id);

  // Create a new, empty tree.  meta_page_id() is what the catalog persists.
  static BPlusTreeIndex create(BufferPoolManager* bpm);
  ...
  class Iterator {
   public:
    Iterator() = default;
    Iterator(BufferPoolManager* bpm, page_id_t leaf, int idx, std::string hi);
    ~Iterator();
