...
constexpr size_t MAX_KEY_LEN = BPlusTreeIndex::MAX_KEY_LEN;       // 64
...
struct PinGuard {
  ...
  page_id_t pid;
  bool dirty = false;
  PinGuard(BufferPoolManager* b, page_id_t p) : bpm(b), pid(p) {}
...
BPlusTreeIndex::BPlusTreeIndex(BufferPoolManager* bpm, page_id_t meta_page_id)
    : bpm_(bpm), meta_page_id_(meta_page_id) {}

BPlusTreeIndex BPlusTreeIndex::create(BufferPoolManager* bpm) {
  page_id_t meta_pid, root_pid;
  Page* mp = bpm->new_page(&meta_pid);
  Page* rp = bpm->new_page(&root_pid);
  if (!mp || !rp) throw std::runtime_error("BPlusTreeIndex::create: no frame available");
  Node root(rp->data());
  root.init_leaf();
  ...
  return BPlusTreeIndex(bpm, meta_pid);
}

page_id_t BPlusTreeIndex::root_page_id() const {
...
void BPlusTreeIndex::set_root_page_id(page_id_t pid) {
  Page* mp = bpm_->fetch_page(meta_page_id_);
  store_u32(mp->data(), static_cast<uint32_t>(pid));
  bpm_->unpin_page(meta_page_id_, true);
}

page_id_t BPlusTreeIndex::find_leaf(std::string_view key) const {
  page_id_t pid = root_page_id();
  while (true) {
    Page* p = bpm_->fetch_page(pid);
    if (!p) throw std::runtime_error("BPlusTreeIndex::find_leaf: fetch failed");
    Node n(p->data());
    if (n.is_leaf()) {
...
std::optional<RID> BPlusTreeIndex::search(std::string_view key) const {
...
bool BPlusTreeIndex::insert(std::string_view key, RID rid) {
  if (key.size() > MAX_KEY_LEN) {
    throw std::invalid_argument("BPlusTreeIndex::insert: key exceeds MAX_KEY_LEN");
  }
  ...
  if (split) {
    ...
    if (!nrp) throw std::runtime_error("BPlusTreeIndex::insert: no frame for new root");
    Node nr(nrp->data());
    nr.init_internal();
    nr.set_num_keys(1);
...
std::optional<BPlusTreeIndex::SplitResult> BPlusTreeIndex::insert_rec(page_id_t node_pid,
                                                           std::string_view key, RID rid,
                                                           bool* inserted_new) {
  Page* p = bpm_->fetch_page(node_pid);
  if (!p) throw std::runtime_error("BPlusTreeIndex::insert_rec: fetch failed");
  ...
  if (n.is_leaf()) {
    ...
    *inserted_new = true;
    ...
    if (!rp) throw std::runtime_error("BPlusTreeIndex: no frame for leaf split");
    Node rn(rp->data());
    rn.init_leaf();
  ...
  page_id_t right_pid;
  Page* rp = bpm_->new_page(&right_pid);
  if (!rp) throw std::runtime_error("BPlusTreeIndex: no frame for internal split");
  Node rn(rp->data());
  rn.init_internal();
...
bool BPlusTreeIndex::erase(std::string_view key) {
  page_id_t leaf = find_leaf(key);
  Page* p = bpm_->fetch_page(leaf);
  Node n(p->data());
  uint16_t nk = n.num_keys();
...
int BPlusTreeIndex::height() const {
  int h = 1;
  page_id_t pid = root_page_id();
  while (true) {
    Page* p = bpm_->fetch_page(pid);
...
BPlusTreeIndex::Iterator BPlusTreeIndex::range(std::string_view lo, std::string_view hi) const {
  page_id_t leaf;
  int idx = 0;
  if (lo.empty()) {
...
BPlusTreeIndex::Iterator::Iterator(BufferPoolManager* bpm, page_id_t leaf, int idx, std::string hi)
...
BPlusTreeIndex::Iterator::~Iterator() { release(); }

void BPlusTreeIndex::Iterator::release() {
  if (page_) {
...
BPlusTreeIndex::Iterator::Iterator(Iterator&& o) noexcept
...
BPlusTreeIndex::Iterator& BPlusTreeIndex::Iterator::operator=(Iterator&& o) noexcept {
  if (this != &o) {
    release();
    bpm_ = o.bpm_;
    page_id_ = o.page_id_;
...
void BPlusTreeIndex::Iterator::load_current() {
  while (page_) {
    Node n(page_->data());
    if (idx_ < n.num_keys()) {
...
void BPlusTreeIndex::Iterator::next() {
  if (!page_) return;
  ++idx_;
  load_current();
}
