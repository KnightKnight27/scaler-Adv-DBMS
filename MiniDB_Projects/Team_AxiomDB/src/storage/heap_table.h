...
// ---------------------------------------------------------------------------
// HeapTable -- an unordered collection of variable-length records, stored as a
// singly-linked list of slotted pages and addressed by RID (page_id, slot).
//
// All page access goes through the BufferPoolManager, so this is where "page manager +
...
class HeapTable {
 public:
  ...
  HeapTable(BufferPoolManager* bpm, page_id_t first_page_id);
  ...
  static std::unique_ptr<HeapTable> create(BufferPoolManager* bpm);
  ...
  class Cursor {
   public:
    ...
    Cursor(BufferPoolManager* bpm, page_id_t start_page);
    ~Cursor();
