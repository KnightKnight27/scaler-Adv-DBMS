...
void LsmMemTable::put(std::string_view key, std::string_view value) {
...
void LsmMemTable::remove(std::string_view key) {
  auto it = table_.find(key);
  if (it == table_.end()) {
...
LsmMemTable::Lookup LsmMemTable::get(std::string_view key, std::string* out) const {
  auto it = table_.find(key);
