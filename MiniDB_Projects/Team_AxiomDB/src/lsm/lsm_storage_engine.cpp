...
LsmStorageEngine::LsmStorageEngine(std::string base_path, size_t threshold)
...
void LsmStorageEngine::load_sstables() {
  ...
  for (auto& [seq, path] : found) {
    sstables_.insert(sstables_.begin(), std::make_unique<LsmSSTable>(path));  // newest at front
    next_seq_ = std::max(next_seq_, seq + 1);
  }
}

void LsmStorageEngine::replay_wal() {
...
void LsmStorageEngine::wal_append(uint8_t op, std::string_view key, std::string_view value) {
  ByteWriter body;
  body.put_u8(op);
  body.put_string(key);
...
Status LsmStorageEngine::put(std::string_view key, std::string_view value) {
...
Status LsmStorageEngine::remove(std::string_view key) {
  wal_append(1, key, {});
  memtable_.remove(key);
  if (memtable_.size_bytes() >= threshold_) flush_memtable();
  return {};
...
std::optional<std::string> LsmStorageEngine::get(std::string_view key) {
  std::string val;
  switch (memtable_.get(key, &val)) {
    case LsmMemTable::Lookup::Found: return val;
    case LsmMemTable::Lookup::Tombstone: return std::nullopt;
    case LsmMemTable::Lookup::Absent: break;
  }
  for (const auto& sst : sstables_) {  // newest -> oldest
    switch (sst->get(key, &val)) {
      case LsmSSTable::Lookup::Found: return val;
      case LsmSSTable::Lookup::Tombstone: return std::nullopt;
      case LsmSSTable::Lookup::Absent: break;
    }
  }
  return std::nullopt;
}

std::unique_ptr<KVIterator> LsmStorageEngine::scan(std::string_view lo, std::string_view hi) {
  ...
  // LsmSSTable only reads the blocks covering [lo, hi) via its sparse index.
...
void LsmStorageEngine::flush() {
  if (!memtable_.empty()) flush_memtable();
}

void LsmStorageEngine::flush_memtable() {
  ...
  LsmSSTable::write(path, entries);
  sstables_.insert(sstables_.begin(), std::make_unique<LsmSSTable>(path));
  bytes_written_ += sstables_.front()->file_size();

  memtable_.clear();
  ::ftruncate(wal_fd_, 0);  // memtable now durable in the LsmSSTable
  ::lseek(wal_fd_, 0, SEEK_SET);

  if (sstables_.size() >= compaction_threshold_) compact();
}

void LsmStorageEngine::compact() {
  ...
  std::vector<LsmSSTable::Iterator> its;
  ...
  sstables_.clear();
  sstables_.push_back(std::make_unique<LsmSSTable>(path));
  bytes_written_ += sstables_.back()->file_size();
  for (const std::string& p : old_paths) ::unlink(p.c_str());
}

uint64_t LsmStorageEngine::disk_size() const {
  uint64_t total = 0;
