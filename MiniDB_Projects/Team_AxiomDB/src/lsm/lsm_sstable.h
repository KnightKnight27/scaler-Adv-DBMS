...
// A key/value (or tombstone) pair handed to the LsmSSTable writer in sorted order.
struct SSTEntry {
  std::string key;
  std::string value;
...
// ---------------------------------------------------------------------------
// LsmSSTable -- a Sorted String Table: an immutable on-disk file of key/value
...
class LsmSSTable {
 public:
  ...
  // Build a new LsmSSTable file at `path` from already-sorted entries.
  static void write(const std::string& path, const std::vector<SSTEntry>& sorted);

  explicit LsmSSTable(std::string path);
  ~LsmSSTable();
