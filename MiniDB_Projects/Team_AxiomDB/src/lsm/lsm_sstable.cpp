...
void LsmSSTable::write(const std::string& path, const std::vector<SSTEntry>& sorted) {
  ByteWriter data;
  ByteWriter index;
  ...
  if (fd < 0) throw std::runtime_error("LsmSSTable::write open: " + std::string(std::strerror(errno)));
  auto write_all = [&](const std::string& s) {
    if (::write(fd, s.data(), s.size()) != static_cast<ssize_t>(s.size()))
      throw std::runtime_error("LsmSSTable::write short write");
  };
  write_all(data.str());
  write_all(index.str());
...
LsmSSTable::LsmSSTable(std::string path) : path_(std::move(path)) {
  fd_ = ::open(path_.c_str(), O_RDONLY);
  if (fd_ < 0) throw std::runtime_error("LsmSSTable open: " + std::string(std::strerror(errno)));
  off_t sz = ::lseek(fd_, 0, SEEK_END);
  if (sz < static_cast<off_t>(kFooterSize)) throw std::runtime_error("LsmSSTable: file too small");
  ...
  if (fr.get_u64() != kMagic) throw std::runtime_error("LsmSSTable: bad magic");
  data_end_ = index_off;
...
LsmSSTable::~LsmSSTable() {
  if (fd_ >= 0) ::close(fd_);
}

std::string LsmSSTable::read_range(uint64_t off, uint64_t len) const {
  std::string buf(len, '\0');
  if (len == 0) return buf;
  if (::pread(fd_, buf.data(), len, static_cast<off_t>(off)) != static_cast<ssize_t>(len))
    throw std::runtime_error("LsmSSTable: short pread");
  return buf;
}

LsmSSTable::Lookup LsmSSTable::get(std::string_view key, std::string* out) const {
...
void LsmSSTable::collect_range(std::string_view lo, std::string_view hi,
...
LsmSSTable::Iterator LsmSSTable::iterator() const {
  return Iterator(read_range(0, data_end_));
}

LsmSSTable::Iterator::Iterator(std::string data) : data_(std::move(data)) { parse(); }

void LsmSSTable::Iterator::parse() {
