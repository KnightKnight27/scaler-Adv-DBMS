...
#include "lsm/lsm_sstable.h"

namespace axiomdb {

// ===========================================================================
// LsmStorageEngine -- LSM Storage Engine: a log-structured merge-tree storage engine implementing
...
//
// Write path:  append to the WAL -> insert into the in-memory LsmMemTable.  When
//   the LsmMemTable exceeds a size threshold it is flushed, in sorted order, to a
//   new immutable LsmSSTable and the WAL is truncated.  Writes are therefore
//   sequential appends -- no in-place random page writes -- which is where LSM
//   wins write throughput.
// Read path:   LsmMemTable first, then SSTables newest -> oldest (newer data
//   shadows older); the first hit wins, a tombstone means "deleted".  Each
//   LsmSSTable's bloom filter + min/max keys let most lookups skip it entirely.
// Delete:      a tombstone write (same as a put); the obsolete value is
//   physically dropped later, during compaction.
// Compaction:  size-tiered -- when the LsmSSTable count reaches a threshold they
//   are merged into one (newest value per key wins, tombstones dropped), which
//   bounds read amplification and reclaims space.
//
// On open, the WAL is replayed into the LsmMemTable (crash recovery for writes that
...
class LsmStorageEngine : public StorageEngine {
 public:
  explicit LsmStorageEngine(std::string base_path, size_t memtable_threshold_bytes = (1u << 20));
  ~LsmStorageEngine() override;
  ...
  // LSM-tree specifics used by the benchmark / tests.
  ...
  // Auto-compaction fires once the LsmSSTable count reaches this many.  Raise it
 ...
 private:
  void flush_memtable();        // LsmMemTable -> new LsmSSTable (+ truncate WAL)
  void load_sstables();         // discover and open existing SSTables on start
  void replay_wal();            // rebuild LsmMemTable from the WAL on start
  ...
  LsmMemTable memtable_;
  std::vector<std::unique_ptr<LsmSSTable>> sstables_;  // index 0 = newest
