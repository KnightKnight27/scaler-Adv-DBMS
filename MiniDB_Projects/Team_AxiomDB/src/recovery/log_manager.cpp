...
LogManager::LogManager(std::string path) : path_(std::move(path)) {
  ...
  if (fd_ < 0) {
    throw std::runtime_error("LogManager: cannot open '" + path_ + "': " + std::strerror(errno));
  }
...
lsn_t LogManager::append(LogType type, txn_id_t txn, uint32_t table_id, std::string_view row) {
  ...
  if (n != static_cast<ssize_t>(bytes.size())) {
    ...
    throw std::runtime_error("LogManager: short write to log");
  }
  return lsn;
}

lsn_t LogManager::log_begin(txn_id_t txn) { return append(LogType::Begin, txn, 0, {}); }
lsn_t LogManager::log_insert(txn_id_t txn, uint32_t table_id, std::string_view row) {
  return append(LogType::Insert, txn, table_id, row);
}
lsn_t LogManager::log_delete(txn_id_t txn, uint32_t table_id, std::string_view row) {
  return append(LogType::Delete, txn, table_id, row);
}
lsn_t LogManager::log_commit(txn_id_t txn) { return append(LogType::Commit, txn, 0, {}); }
lsn_t LogManager::log_abort(txn_id_t txn) { return append(LogType::Abort, txn, 0, {}); }
...
void LogManager::truncate() {
  if (::ftruncate(fd_, 0) != 0) {
    throw std::runtime_error("LogManager: ftruncate failed: " + std::string(std::strerror(errno)));
  }
  next_lsn_ = 0;
}
