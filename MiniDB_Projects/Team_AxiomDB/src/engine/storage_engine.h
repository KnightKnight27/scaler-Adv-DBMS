...
//
//   * ClassicStorageEngine -- slotted-page heap file + buffer pool + B+tree index
//                        (the required "classic" core).
//   * LsmStorageEngine       -- LsmMemTable + SSTables + compaction (LSM Storage Engine).
//
// Defining this seam *before* writing either implementation is what lets the
// LSM Storage Engine A/B benchmark be a one-line swap instead of a fork, and is what the
