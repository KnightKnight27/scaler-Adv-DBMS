...
ClassicStorageEngine::ClassicStorageEngine(const std::string& path, size_t buffer_frames)
  ...
  if (disk_.num_pages() == 0) {
    ...
    auto heap = HeapTable::create(&pool_);
    BPlusTreeIndex index = BPlusTreeIndex::create(&pool_);
    ...
    disk_.sync();
    heap_ = std::move(heap);
    index_ = std::make_unique<BPlusTreeIndex>(&pool_, index.meta_page_id());
  } else {
    ...
    heap_ = std::make_unique<HeapTable>(&pool_, heap_first);
    index_ = std::make_unique<BPlusTreeIndex>(&pool_, index_meta);
  }
}

ClassicStorageEngine::~ClassicStorageEngine() {
  pool_.flush_all();
  disk_.sync();
}

Status ClassicStorageEngine::put(std::string_view key, std::string_view value) {
...
std::optional<std::string> ClassicStorageEngine::get(std::string_view key) {
  auto rid = index_->search(key);
  if (!rid) return std::nullopt;
  return heap_->get(*rid);
}

Status ClassicStorageEngine::remove(std::string_view key) {
  auto rid = index_->search(key);
  if (!rid) return {};
  index_->erase(key);
  heap_->erase(*rid);
...
std::unique_ptr<KVIterator> ClassicStorageEngine::scan(std::string_view lo, std::string_view hi) {
...
void ClassicStorageEngine::flush() {
  pool_.flush_all();
  disk_.sync();
}

uint64_t ClassicStorageEngine::disk_size() const {
  struct stat st {};
