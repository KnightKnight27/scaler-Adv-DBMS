...
// ---------------------------------------------------------------------------
// BufferPoolManager - Caches page frames in memory in memory frames and
// mediates ALL page access for the layers above (heap file, B+tree).
//
// Core mechanics (the M1 module: "page manager + buffer pool integrated"):
...
class BufferPoolManager {
 public:
  BufferPoolManager(DiskStorageManager* disk, size_t pool_size = BUFFER_POOL_DEFAULT_FRAMES,
             size_t k = 2);
  ...
  DiskStorageManager* disk() { return disk_; }
