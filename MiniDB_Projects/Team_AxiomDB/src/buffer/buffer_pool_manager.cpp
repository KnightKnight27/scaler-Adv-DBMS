...
BufferPoolManager::BufferPoolManager(DiskStorageManager* disk, size_t pool_size, size_t k)
...
frame_id_t BufferPoolManager::grab_frame() {
  if (!free_list_.empty()) {
...
Page* BufferPoolManager::new_page(page_id_t* out_pid) {
...
Page* BufferPoolManager::fetch_page(page_id_t pid) {
  std::lock_guard<std::mutex> guard(latch_);
...
bool BufferPoolManager::unpin_page(page_id_t pid, bool is_dirty) {
  std::lock_guard<std::mutex> guard(latch_);
  auto it = page_table_.find(pid);
...
bool BufferPoolManager::flush_page(page_id_t pid) {
  std::lock_guard<std::mutex> guard(latch_);
  auto it = page_table_.find(pid);
  if (it == page_table_.end()) return false;
