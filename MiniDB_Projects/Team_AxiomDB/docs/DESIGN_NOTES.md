# AxiomDB — Design Decisions & Trade-offs

This document records the engineering decisions behind AxiomDB and the
trade-offs each one makes — what is prioritized over what, and why the choice is
sound. AxiomDB is a single-process relational database engine with an LSM-tree
storage extension (LSM Storage Engine).
...
2. **A single storage abstraction is prioritized over specialization.** The
   ...
   abstraction and the LSM-tree comparison is an engine swap rather than a fork.
...
## LSM-Tree Extension — LSM-tree storage
...
in-memory LsmMemTable; when the LsmMemTable fills, it is flushed in key order to an
immutable LsmSSTable. This write path prioritizes **write throughput** (sequential
appends, no in-place random page writes) over read-path simplicity.

The LsmMemTable is a **balanced tree (`std::map`)** rather than a skip list. A
...
Each LsmSSTable carries a **sparse index** (one entry every Nth key) and a **bloom
...
| LSM `std::map` LsmMemTable | correctness, simplicity | skip-list authenticity | natural future substitution |
