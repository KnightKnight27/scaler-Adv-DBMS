...
// this is exactly what makes the LSM vs Heap B+Tree Benchmark a clean A/B swap.
...
TEST_CASE("ClassicStorageEngine satisfies the StorageEngine contract", "[engine][kv]") {
  std::string dir = "/tmp/wdb_kv_heap_" + std::to_string(::getpid());
  fs::remove_all(dir);
  fs::create_directories(dir);
  {
    ClassicStorageEngine eng(dir + "/db.wdb");
...
TEST_CASE("LsmStorageEngine satisfies the StorageEngine contract", "[engine][kv]") {
  std::string dir = "/tmp/wdb_kv_lsm_" + std::to_string(::getpid());
  fs::remove_all(dir);
  fs::create_directories(dir);
  {
    LsmStorageEngine eng(dir + "/db", 8192);
