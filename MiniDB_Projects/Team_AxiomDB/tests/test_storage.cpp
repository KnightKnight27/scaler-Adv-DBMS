...
TEST_CASE("SlottedPageLayout insert / get / free-space", "[storage][page]") {
  std::array<char, PAGE_SIZE> buf{};
  SlottedPageLayout page(buf.data());
  page.init();
...
TEST_CASE("SlottedPageLayout erase tombstones without disturbing neighbours", "[storage][page]") {
  std::array<char, PAGE_SIZE> buf{};
  SlottedPageLayout page(buf.data());
  page.init();
...
TEST_CASE("SlottedPageLayout update_in_place requires equal length", "[storage][page]") {
  std::array<char, PAGE_SIZE> buf{};
  SlottedPageLayout page(buf.data());
  page.init();
...
TEST_CASE("SlottedPageLayout reports being full", "[storage][page]") {
  std::array<char, PAGE_SIZE> buf{};
  SlottedPageLayout page(buf.data());
  page.init();
...
TEST_CASE("SlottedPageLayout next_page_id and lsn round-trip", "[storage][page]") {
  std::array<char, PAGE_SIZE> buf{};
  SlottedPageLayout page(buf.data());
  page.init();
...
TEST_CASE("DiskStorageManager allocates, persists, and reopens", "[storage][disk]") {
