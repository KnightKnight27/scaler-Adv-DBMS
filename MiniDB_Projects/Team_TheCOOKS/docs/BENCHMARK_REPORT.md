# WALterDB Benchmark Report — Track C: LSM-tree vs Heap-file + B+tree

## 1. Objective

Compare WALterDB's two storage engines on the metrics the capstone requires for
Track C: **write throughput, read latency, and storage amplification** (plus
...
(`bench/benchmark.cpp`) runs the **exact same workload** against each — the
...
| | |
|---|---|
| Engines | `HeapBTreeEngine` (slotted-page heap file + B+tree index, via the buffer pool) vs `LSMEngine` (MemTable + SSTables + bloom filters + size-tiered compaction) |
...
| Buffer pool | 4096 frames (16 MB) for the heap engine; LSM MemTable threshold 1 MiB |
...
writes are appends to an in-memory MemTable plus a sequential WAL append; the
...
- We expected the B+tree to win reads, "especially as the LSM accumulates
  un-compacted SSTables." It didn't, for two reasons: (1) every SSTable carries
  a **bloom filter** + min/max keys, so a lookup probes essentially one SSTable,
...
- Compaction barely moved read latency (1.04→1.04 µs) **because the bloom
  filters had already neutralised the multi-SSTable penalty** — a consistent,
  honest finding.
...
writes 2.23× the logical bytes (WAL + one SSTable pass), versus 4.81× for the
...
./build/walterdb_bench 100000 20000   # N rows, M point-lookups
