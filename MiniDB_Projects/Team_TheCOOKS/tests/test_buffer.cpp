...
TEST_CASE("BufferPool flushes the dirty version on eviction", "[buffer][m1]") {
  std::string path = temp_db_path("bp_evict");
  ::remove(path.c_str());
  {
    DiskManager dm(path);
    BufferPool bp(&dm, /*pool_size=*/2, /*k=*/2);
    ...
    REQUIRE(f1 != nullptr);
    {
      SlottedPage s0(f0->data()); s0.init(); s0.insert("page-zero-data");
      SlottedPage s1(f1->data()); s1.init(); s1.insert("page-one-data");
    }
  ...
  // Reopen and confirm both original pages persisted their *modified* content.
  {
    ...
    REQUIRE(SlottedPage(buf.data()).get(0).value() == "page-zero-data");
    REQUIRE(dm.read_page(1, buf.data()).ok());
    REQUIRE(SlottedPage(buf.data()).get(0).value() == "page-one-data");
  }
  ::remove(path.c_str());
}

TEST_CASE("BufferPool returns nullptr when every frame is pinned", "[buffer]") {
