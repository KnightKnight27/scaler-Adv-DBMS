...
TEST_CASE("B+tree insert forces multi-level splits; search finds all", "[btree]") {
  ...
  ::remove(path.c_str());
  DiskManager dm(path);
  BufferPool bp(&dm, /*pool_size=*/64, /*k=*/2);  // << number of pages -> real paging

  BPlusTree tree = BPlusTree::create(&bp);
...
TEST_CASE("B+tree persists across reopen via meta page", "[btree]") {
  ...
  }
  {
    DiskManager dm(path);
    BufferPool bp(&dm, 64, 2);
    BPlusTree tree(&bp, meta);  // reopen from persisted meta page
    REQUIRE(tree.height() >= 3);
...
TEST_CASE("B+tree rejects oversized keys", "[btree]") {
  ...
  BufferPool bp(&dm, 16, 2);
  BPlusTree tree = BPlusTree::create(&bp);
  std::string huge(BPlusTree::MAX_KEY_LEN + 1, 'x');
