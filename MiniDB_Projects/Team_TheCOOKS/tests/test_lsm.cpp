...
TEST_CASE("LSM flushes to SSTables and survives reopen", "[lsm]") {
  ...
  const int N = 2000;
  {
    LSMEngine lsm(d + "/db", /*threshold=*/4096);  // tiny -> forces many flushes
    for (int i = 0; i < N; ++i) lsm.put(key_of(i), "v" + std::to_string(i));
    REQUIRE(lsm.num_sstables() >= 1);
  }
  {
    LSMEngine lsm(d + "/db", 4096);  // reopen: load SSTables + replay WAL
    for (int i = 0; i < N; ++i) {
...
TEST_CASE("LSM newest value wins across memtable and SSTables", "[lsm]") {
  ...
  {
    LSMEngine lsm(d + "/db", 4096);
    for (int i = 0; i < 500; ++i) lsm.put(key_of(i), "old");
    lsm.flush();                                   // push to an SSTable
...
TEST_CASE("LSM compaction reduces SSTable count and reclaims space", "[lsm][compaction]") {
  std::string d = make_dir("compact");
  {
    LSMEngine lsm(d + "/db", 1u << 20);
