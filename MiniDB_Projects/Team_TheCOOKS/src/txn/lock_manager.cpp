...
bool LockManager::lock_shared(Transaction* txn, uint64_t resource) {
  return acquire(txn, resource, LockMode::Shared);
}

bool LockManager::lock_exclusive(Transaction* txn, uint64_t resource) {
...
bool LockManager::can_grant(const ResourceQueue& rq, txn_id_t txn, LockMode mode) const {
...
bool LockManager::has_cycle(txn_id_t start) const {
...
bool LockManager::acquire(Transaction* txn, uint64_t resource, LockMode mode) {
