...
Table::Table(BufferPool* bpm, TableInfo* info) : bpm_(bpm), info_(info) {
  heap_ = std::make_unique<HeapFile>(bpm, info->heap_first_page);
  if (info->has_primary_index()) {
    index_ = std::make_unique<BPlusTree>(bpm, info->index_meta_page);
  }
}
