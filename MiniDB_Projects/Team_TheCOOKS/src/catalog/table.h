...
class Table {
 public:
  Table(BufferPool* bpm, TableInfo* info);
  ...
  BPlusTree* index() { return index_.get(); }  // nullptr if the table has no PK

 private:
  BufferPool* bpm_;
  TableInfo* info_;
  std::unique_ptr<HeapFile> heap_;
  std::unique_ptr<BPlusTree> index_;  // primary-key index, nullptr if none
};
