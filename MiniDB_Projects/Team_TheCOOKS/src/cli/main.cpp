...
//
//   walterdb [db_path]      (default db_path: "walterdb")
// ===========================================================================
#include <unistd.h>
...
int main(int argc, char** argv) {
  std::string db_path = argc > 1 ? argv[1] : "walterdb";
  Database db(db_path);
  bool interactive = ::isatty(STDIN_FILENO);

  if (interactive) {
    std::cout << "WALterDB -- a mini relational database (Track C: LSM).\n"
  ...
  bool crash = false;
  while (true) {
    if (interactive) std::cout << (buffer.empty() ? "walterdb> " : "      ...> ") << std::flush;
