...
size_t HeapFile::max_record_size() {
  // One page minus the header and one slot.
  return PAGE_SIZE - SlottedPage::kHeaderSize - SlottedPage::kSlotSize;
}

HeapFile::HeapFile(BufferPool* bpm, page_id_t first_page_id)
  ...
  while (pid != INVALID_PAGE_ID) {
    ...
    if (!p) throw std::runtime_error("HeapFile: cannot fetch page while finding tail");
...
std::unique_ptr<HeapFile> HeapFile::create(BufferPool* bpm) {
  page_id_t pid;
  Page* p = bpm->new_page(&pid);
  if (!p) throw std::runtime_error("HeapFile::create: no frame available");
  SlottedPage sp(p->data());
  sp.init();
  bpm->unpin_page(pid, true);
  return std::make_unique<HeapFile>(bpm, pid);
}

RID HeapFile::insert(std::string_view record) {
  if (record.size() > max_record_size()) {
    throw std::runtime_error("HeapFile::insert: record exceeds page capacity (no overflow pages)");
  }

  // Try the current tail page first.
  Page* tail = bpm_->fetch_page(last_page_id_);
  if (!tail) throw std::runtime_error("HeapFile::insert: cannot fetch tail page");
  {
    SlottedPage sp(tail->data());
  ...
  if (!np) throw std::runtime_error("HeapFile::insert: no frame for new page");
  slot_id_t new_slot;
  {
  ...
  if (!prev) throw std::runtime_error("HeapFile::insert: cannot re-fetch tail page to link");
  {
    SlottedPage psp(prev->data());
...
std::optional<std::string> HeapFile::get(RID rid) const {
...
bool HeapFile::update_in_place(RID rid, std::string_view record) {
  Page* p = bpm_->fetch_page(rid.page_id);
  if (!p) return false;
  SlottedPage sp(p->data());
...
HeapFile::Cursor::Cursor(BufferPool* bpm, page_id_t start_page) : bpm_(bpm) {
...
HeapFile::Cursor::~Cursor() { release(); }

void HeapFile::Cursor::release() {
  if (page_) {
...
HeapFile::Cursor::Cursor(Cursor&& o) noexcept
...
HeapFile::Cursor& HeapFile::Cursor::operator=(Cursor&& o) noexcept {
  if (this != &o) {
    release();
    bpm_ = o.bpm_;
    page_id_ = o.page_id_;
...
void HeapFile::Cursor::seek_live() {
