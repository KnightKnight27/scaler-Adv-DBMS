// ===========================================================================
// WALterDB Track-C benchmark: heap-file + B+tree  vs  LSM-tree.
//
...
//
//   ./walterdb_bench [N rows] [M point-lookups]
// ===========================================================================
#include <unistd.h>
...
int main(int argc, char** argv) {
  ...
  std::string dir = "/tmp/walterdb_bench_" + std::to_string(::getpid());
  fs::remove_all(dir);
  ...
  std::printf("WALterDB Track-C benchmark | N=%d rows, M=%d point-lookups, value=%zuB\n\n",
              N, M, kValueSize);
  ...
  {
    HeapBTreeEngine eng(dir + "/heap.wdb");
    auto t0 = Clock::now();
