...
RecoveryResult Recovery::runRedo(const std::string& wal_path, TableProvider table_provider) {
    ...
    if (fd < 0) {
        std::cout << "[Recovery] No WAL file found at '" << wal_path
                  << "' — starting fresh.\n";
        return result;
    }
    ...
    // ── Pass 1: Analysis — find all committed TxIDs ──────────────────────────
    ...
    while (::read(fd, &rec, WAL_RECORD_SIZE) == static_cast<ssize_t>(WAL_RECORD_SIZE)) {
        ...
        if (rec.lsn > result.last_lsn) result.last_lsn = rec.lsn;
    ...
    // ── Pass 2: Redo — replay INSERT/DELETE for committed txns ───────────────
    std::cout << "[Recovery] Pass 2: replaying committed transactions...\n";

    for (const WALRecord& r : all_records) {
        if (r.type != WALRecordType::INSERT && r.type != WALRecordType::DELETE) {
            continue;
        }

        if (!committed.count(r.txid)) {
            continue;  // skip non-committed records
        }
        ...
        if (r.type == WALRecordType::INSERT) {
            ...
            // Find the record by key and delete it
            if (index) {
                auto rid_opt = index->search(r.key);
                if (rid_opt) {
