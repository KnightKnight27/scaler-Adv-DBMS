...
* In addition to writing its local WAL (`minidb.wal`), it ships committed transactions to a shared log file `minidb_replication.log` using the `Primary::shipLog` mechanism.
...
* Runs a background thread that polls `minidb_replication.log` every 200 milliseconds.
...
1. **Startup**: The script deletes any old databases and creates named pipes (`pri_pipe` and `rep_pipe`) to control the processes in the background.
