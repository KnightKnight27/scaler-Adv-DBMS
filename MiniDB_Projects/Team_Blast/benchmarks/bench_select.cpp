...
// ─── Select Latency Benchmark: Table Scan vs Index Scan ──────────────────────
// Inserts N records, then compares lookup time:
//   (a) sequential table scan for key=TARGET
//   (b) index scan for key=TARGET
...
int main() {
    ...
    for (int i = 1; i <= N; ++i) {
        ...
        if (rid.isValid()) index.insert(i, rid);
        wal.logCommit(txid);
        txm.commit(txid);
    }
    ...
    for (int r = 0; r < RUNS; ++r) {
        heap.scanAll([&](const RecordID&, const Record& rec) {
            if (rec.key == TARGET) { scan_hits++; return false; }
            return true;
        });
    }
