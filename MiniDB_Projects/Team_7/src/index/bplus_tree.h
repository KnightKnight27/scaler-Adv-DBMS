...
class BPlusTree {
 public:
  ...
  BPlusTree(BufferPool *bpm, page_id_t root_page_id = INVALID_PAGE_ID)
  ...
  // Insert key->rid. Returns false if the key already exists (no duplicates).
 ...
 private:
  ...
  page_id_t FindLeaf(const IndexKey &key) const;  // descend to the target leaf
