...
bool SeqScanNode::Next(OutputRow& out) {
    if (cursor_ >= (int)rows_.size()) return false;
    out = rows_[cursor_++];
    return true;
}

void SeqScanNode::Close() { rows_.clear(); cursor_ = 0; }

// ============================================================
// IndexScanNode
// ============================================================

IndexScanNode::IndexScanNode(BPlusTree* index, HeapFile* heap, const Schema& schema,
                             int low_key, int high_key, bool exact)
    : index_(index), heap_(heap), schema_(schema),
      low_key_(low_key), high_key_(high_key), exact_(exact) {}
...
void IndexScanNode::Close() { rows_.clear(); cursor_ = 0; }

// ============================================================
// FilterNode
// ============================================================

FilterNode::FilterNode(std::unique_ptr<PlanNode> child, ExprPtr predicate, const Schema& schema)
...
bool FilterNode::Next(OutputRow& out) {
    while (child_->Next(out)) {
        if (EvaluateExpr(pred_, out.values, schema_)) return true;
    }
    return false;
}
...
ProjectionNode::ProjectionNode(std::unique_ptr<PlanNode> child, std::vector<int> col_indices)
...
bool ProjectionNode::Next(OutputRow& out) {
    ...
    for (int i : col_indices_) {
        if (i < (int)out.values.size()) projected.push_back(out.values[i]);
    }
        ...
        if (inner_->Next(inner_row)) {
            ...
            if (outer_col_ < (int)current_outer_.values.size() &&
                ...
                out.values.insert(out.values.end(),
                                  inner_row.values.begin(), inner_row.values.end());
...
Value EvalValue(const ExprPtr& expr, const Row& row, const Schema& schema) {
    if (expr->type == Expr::LITERAL) return expr->value;
...
Value EvalValueJoin(const ExprPtr& expr, const Row& row,
                    const Schema& ls, const std::string& lt,
                    const Schema& rs, const std::string& rt) {
    if (expr->type == Expr::LITERAL) return expr->value;
    if (expr->type == Expr::COLUMN_REF) {
        ...
        if (expr->table_name == rt || expr->table_name.empty()) {
            ...
            if (idx >= 0) return row[ls.columns.size() + idx];
        }
        throw std::runtime_error("Unknown column: " +
            (expr->table_name.empty() ? "" : expr->table_name + ".") + expr->column_name);
    }
    throw std::runtime_error("Cannot evaluate expression as value");
}
