...
static void CollectConditions(const ExprPtr& expr, std::vector<ExprPtr>& conditions) {
    if (!expr) return;
...
IndexCondition AnalyzeWhere(const ExprPtr& where, const Schema& schema) {
    IndexCondition cond;
    if (!where || schema.pk_index < 0) return cond;
    ...
    for (auto& c : all_conditions) {
        ...
        bool is_pk_left = (c->left->type == Expr::COLUMN_REF && c->left->column_name == pk_name);
        bool is_lit_right = (c->right->type == Expr::LITERAL && c->right->value.type == DataType::INT);

        if (is_pk_left && is_lit_right) {
            int val = c->right->value.int_val;
            if (c->op == "=") { has_eq = true; eq_val = val; }
            else if (c->op == ">") { low = std::max(low, val + 1); }
            else if (c->op == ">=") { low = std::max(low, val); }
            else if (c->op == "<") { high = std::min(high, val - 1); }
            else if (c->op == "<=") { high = std::min(high, val); }
            else { remaining.push_back(c); continue; }
...
std::unique_ptr<PlanNode> CreateSelectPlan(const SelectStmt& stmt, HeapEngine* engine) {
    ...
    if (!stmt.join.has_value()) {
        ...
        if (idx_cond.can_use_index && index != nullptr) {
            ...
            double index_cost = 3 + sel * stats.num_pages;  // tree height ~3 + fraction of pages
            double seq_cost = stats.num_pages;

            std::cout << "[Optimizer] SeqScan cost=" << seq_cost
                      << ", IndexScan cost=" << index_cost
                      << " → choosing " << (index_cost < seq_cost ? "IndexScan" : "SeqScan") << "\n";

            if (index_cost < seq_cost) {
                if (idx_cond.is_exact) {
                    plan = std::make_unique<IndexScanNode>(
                        index, heap, schema, idx_cond.exact_key, idx_cond.exact_key, true);
                } else {
                    plan = std::make_unique<IndexScanNode>(
                        index, heap, schema, idx_cond.range_low, idx_cond.range_high, false);
                }
        ...
        std::string left_tbl = join.left_col.table.empty() ? stmt.table_name : join.left_col.table;
        std::string right_tbl = join.right_col.table.empty() ? join.table_name : join.right_col.table;
        ...
        std::cout << "[Optimizer] JOIN: " << stmt.table_name << "(" << stats.row_count << " rows)"
                  << " ⋈ " << join.table_name << "(" << right_stats.row_count << " rows)"
                  << " → outer=" << (stats.row_count <= right_stats.row_count ? stmt.table_name : join.table_name) << "\n";
        ...
        if (stats.row_count <= right_stats.row_count) {
            ...
            o_col = left_col; i_col = right_col;
        } else {
            outer = std::move(right_scan);
            inner = std::move(left_scan);
            o_col = right_col; i_col = left_col;
        }

        plan = std::make_unique<NestedLoopJoinNode>(std::move(outer), std::move(inner), o_col, i_col);
        ...
        if (stmt.where_clause) {
            ...
            combined.columns.insert(combined.columns.end(),
                                    right_schema.columns.begin(), right_schema.columns.end());
            plan = std::make_unique<FilterNode>(std::move(plan), stmt.where_clause, combined);
        }
    }
