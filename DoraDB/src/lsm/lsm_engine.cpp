...
std::vector<Row> LSMEngine::Get(const std::string& table, int key) {
    ...
    if (ts.memtable.IsDeleted(key)) return {};  // tombstone
    ...
    for (auto& sst : ts.sstables) {
        ...
        if (entry) {
            ...
            return {DeserializeRow(entry->data.data(), entry->data.size(), ts.schema)};
        }
    }
    return {};
}
...
std::vector<Row> LSMEngine::Scan(const std::string& table) {
    ...
    for (auto& sst : ts.sstables) {
        for (auto& e : sst->GetEntries()) {
            if (latest.count(e.key) || deleted.count(e.key)) continue;
            if (e.deleted) { deleted[e.key] = true; continue; }
...
std::vector<Row> LSMEngine::RangeScan(const std::string& table, int low, int high) {
    ...
    for (auto& row : all) {
        if (ts.schema.pk_index >= 0) {
            ...
            if (pk >= low && pk <= high) results.push_back(row);
        }
    }
    return results;
}
...
void LSMEngine::FlushMemTable(const std::string& table) {
    ...
    std::cout << "[LSM] Flushed MemTable to " << path
