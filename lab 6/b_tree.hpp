...
// Each node holds between t-1 and 2t-1 keys (except root)
template <typename Key, typename Value, typename Compare = std::less<Key>>
class b_tree {
public:
    explicit b_tree(int t = 3) : t_(t < 2 ? 2 : t) {}

    // no copy or move - keep it simple
    ...
    // visits every key-value pair in sorted order
    template <typename F>
    ...
    // returns empty string if tree is valid, error message otherwise
...
private:
    ...
    bool insert_nonfull(Node* node, const Key& key, const Value& value) {
        ...
        if (node->children[idx]->is_full(t_)) {
            ...
            if (equal_keys(node->keys[idx], key)) { node->values[idx] = value; return false; }
    ...
    bool erase_from(Node* node, const Key& key) {
        ...
        if (found_here) return erase_internal(node, idx, key);
    ...
    int fix_child_before_descent(Node* parent, int idx) {
        ...
        if (right) { merge_children(parent, idx);     return idx; }
        merge_children(parent, idx - 1); return idx - 1;
    }
    ...
    std::string verify_rec(const Node* node, bool is_root, int depth,
                           int& leaf_depth, const Key* low, const Key* high) const {
        int n = node->key_count();
        ...
        for (int i = 0; i <= n; ++i) {
            ...
            std::string err = verify_rec(node->children[i], false, depth + 1,
                                         leaf_depth, child_low, child_high);
