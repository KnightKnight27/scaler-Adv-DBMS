...
// Tokenize: numbers, identifiers, two-char operators, single-char operators, parens
...
// Shunting-Yard: infix token list -> postfix (RPN) token list
...
// Evaluate RPN given a variable map (all numeric values as doubles)
...
// Extract numeric value from a Row column (for expression evaluation)
...
SelectQuery parse_select(const std::string& sql) {
    ...
    while (ss >> word) {
        ...
        if (kw == "WHERE") {
            ...
            while (ss >> w2) {
                std::string kw2 = to_upper(w2);
                if (kw2 == "ORDER" || kw2 == "LIMIT") { word = w2; goto next_clause; }
...
std::vector<Row> execute(const SelectQuery& q, const std::vector<Row>& data) {
    ...
    for (const auto& row : data) {
        ...
        // Project columns (SELECT *)  or specific columns
        if (q.columns.empty()) {
...
int main() {
    ...
    // Pre-fetched data — simulates what a storage layer returns after page reads
