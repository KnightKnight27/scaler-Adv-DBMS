...
// ─────────────────────────────────────────────────────────────────────
//  TxnEngine — in-memory transaction manager combining MVCC + Strict 2PL
//
//  • MVCC for reads: writes never overwrite in place; they append a new
//    version. A reader walks the per-key chain and keeps the first one
//    visible to its own snapshot.
//  • Strict 2PL for writes: exclusive lock held until commit/abort.
//  • Deadlock detection via DFS over a waits-for graph.
// ─────────────────────────────────────────────────────────────────────
class TxnEngine {
public:
    TxnId start() {
    ...
    void store_(TxnId tx, const RowKey& k, const string& data) {
        ...
        // SI lost-update check: reject if another committed tx touched
        // this row after our snapshot
    ...
    // Garbage collection: prune dead versions older than oldest active snapshot
    size_t gc() {
        Stamp horizon;
        {
            lock_guard<mutex> lk(txnMu);
...
private:
    ...
    // A version is visible if its creator is visible AND its invalidator is NOT visible
    ...
    // First-updater-wins check: reject writes if someone committed after our snapshot
    ...
    // Strict 2PL: lock acquisition with deadlock detection
    void lockRow(TxnId tx, const RowKey& k, LockKind kind) {
        ...
        while (true) {
            ...
            // Lock upgrade: S → X when sole holder
    ...
    // DFS over waits-for graph; on cycle, kill the youngest (highest-id) tx
...
// ─────────────────────────────────────────────────────────────────────
//  Demo functions exercising every feature of the engine
// ─────────────────────────────────────────────────────────────────────
...
int main() {
    cout << "=== Transaction Manager (Piyush Pawan Kumar, 24BCS10296) ===" << endl;
    TxnEngine engine;
    demoSnapshot(engine);
