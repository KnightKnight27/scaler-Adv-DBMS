...
### Behavior:
- Creates `input.txt` (if not already present) with seed data.
- Opens the input file for reading.
- Opens/creates the output file for writing (truncating existing content).
- Copies data from input to output using a 4KB buffer with `read()` + `write()`.
- Closes all file descriptors and reports success.

---

## Build and Run

The project includes a `Makefile` for ease of use.
