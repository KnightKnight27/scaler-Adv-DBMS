...
class DatabaseClosedError(RuntimeError):
    """A table or Database operation ran after the handle crashed/closed."""
...
class _BaseTable:
    """Lock helpers shared by the heap and LSM tables: take a row/table lock and
    record it on the txn so 2PL can release everything at commit/abort."""
    ...
    def _xlock(self, txn, res):
        self.db.locks.acquire(txn.txn_id, res, EXCLUSIVE)
        txn.locks.add(res)

    def _slock(self, txn, res):
        self.db.locks.acquire(txn.txn_id, res, SHARED)
        txn.locks.add(res)
...
class Table(_BaseTable):
    ...
    def insert(self, txn: Transaction, row):
        ...
        key = row[self.pk]
        # table X lock so a concurrent scan can't miss the new row (phantoms)
        self._xlock(txn, self._table_res())
        self._xlock(txn, self._row_res(key))
        if self.index.search(key) is not None:
            raise DuplicateKeyError(f"duplicate primary key {key!r} in {self.name}")
    ...
    def get_by_key(self, txn: Transaction, key):
        self.db._ensure_open()
        self._slock(txn, self._row_res(key))
        rid = self.index.search(key)
        if rid is None:
            return None
    ...
    def seq_scan(self, txn: Transaction):
        self.db._ensure_open()
        self._slock(txn, self._table_res())
        for rid, rec in self.heap.scan():
            row = self.schema.deserialize(rec)
            self._slock(txn, self._row_res(row[self.pk]))
            yield row
    ...
    # Recovery replays log records directly: no locks, no re-logging.
...
class LSMTable(_BaseTable):
    ...
    def insert(self, txn: Transaction, row):
        self.db._ensure_open()
        key = row[self.pk]
        self._xlock(txn, self._table_res())     # table X: block phantoms on scan
        self._xlock(txn, self._row_res(key))
        if self.index.search(key) is not None:
            raise DuplicateKeyError(f"duplicate primary key {key!r} in {self.name}")
        self.db.wal.log_insert(txn.txn_id, self.name, key, row)
    ...
    # Recovery replays directly: no locks, no re-logging.
    def redo_insert(self, key, row):
        if self.index.search(key) is not None:
            return
        self.lsm.put(key, self.schema.serialize(tuple(row)))
    ...
    def _ensure_open(self):
        if self._crashed:
            raise DatabaseClosedError(
                "database instance has crashed; reopen it before use")
    ...
    def checkpoint(self):
        ...
        for table in self.tables.values():
            ...
            if flush is not None:
                flush()
        self.bp.flush_all()
        self.disk.sync()                    # force data pages down before we drop the redo log
        self.catalog.save()
    ...
    def close(self):
        ...
        for table in self.tables.values():
            ...
            if flush is not None:
                flush()
        self.bp.flush_all()
        self.disk.sync()                    # clean shutdown: data durable without per-write fsync
        self.catalog.save()
        self.disk.close()
        self.wal.close()
