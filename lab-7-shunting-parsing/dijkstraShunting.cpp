...
std::vector<std::string> tokenize(const std::string &query) {
  ...
  while (i < n) {
    ...
    if ((query[i] == '>' || query[i] == '<' || query[i] == '!') && i + 1 < n &&
        query[i + 1] == '=') {
  ...
  for (const auto &tok : postfix) {
    ...
    if (st.size() < 2) {
      throw std::runtime_error(
          "Malformed expression: not enough operands for '" + tok + "'");
    }
    int rhs = st.top();
    st.pop();
    int lhs = st.top();
    ...
    if (tok == ">")
      ...
      throw std::runtime_error("Unsupported operator: '" + tok + "'");
  }
  if (st.size() != 1) {
    throw std::runtime_error(
        "Malformed expression: stack does not collapse to a single value");
  }
  return st.top() != 0;
}
