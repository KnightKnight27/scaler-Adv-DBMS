...
public:
  ...
  ParsedQuery parseQuery() {
    ...
    if (cursor_ != tokens_.size())
      throw std::runtime_error("Unexpected token after WHERE clause: '" +
                               tokens_[cursor_] + "'");
...
private:
  ...
  void expectKeyword(const std::string &kw) {
    ...
    if (got != kw)
      throw std::runtime_error("Expected " + kw + " keyword, got '" + got +
                               "'");
  }
...
bool evaluate(const Node *node, const Employee &e) {
  ...
  if (node->op == "AND")
    return evaluate(node->l.get(), e) && evaluate(node->r.get(), e);
  if (node->op == "OR")
    return evaluate(node->l.get(), e) || evaluate(node->r.get(), e);
...
void runQuery(const std::string &sql, const std::vector<Employee> &employees) {
  ...
  for (const auto &e : employees) {
    if (!evaluate(query.whereRoot.get(), e))
      continue;
...
int main() {
  try {
    ...
    runQuery("SELECT name FROM employees WHERE id > 3 AND age >= 30",
             employees);
    runQuery(
