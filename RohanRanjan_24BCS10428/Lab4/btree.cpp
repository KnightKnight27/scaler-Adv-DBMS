...
//
// Every node (except the root) holds between T-1 and 2T-1 keys and, if
// internal, between T and 2T children. This is the on-disk index structure
// used by PostgreSQL, MySQL/InnoDB, and SQLite (a node maps to one page).
//
// Operations: insert (split full child on the way down), search, and
// delete (borrow from a sibling / merge with a sibling / replace with
...
const int T = 2;   // minimum degree; raise to increase fanout
...
class BTree {
    ...
    // Split a FULL child (2T-1 keys) of `parent` at index i:
    // the median key moves up into the parent, and the right half of the
    // keys/children moves into a new node z.
    void split_child(BNode* parent, int i) {
        BNode* y = parent->children[i];
        BNode* z = new BNode();
        z->leaf = y->leaf;

        int med = y->keys[T - 1];                 // median promoted to parent

        z->keys.assign(y->keys.begin() + T, y->keys.end());   // right half -> z
        y->keys.resize(T - 1);                                // left half stays in y
    ...
    // Ensure children[idx] has at least T keys before we descend into it.
    ...
    void delete_key(BNode* node, int key) {
        ...
        if (idx < (int)node->keys.size() && node->keys[idx] == key) {
            ...
            if ((int)node->children[idx]->keys.size() < T)
                fill(node, idx);
            // After fill, idx may have merged into the previous child.
...
public:
    ...
    void remove(int key) {
        if (!root) return;
        delete_key(root, key);
        if (root->keys.empty() && !root->leaf) {      // shrink height
            BNode* old = root;
