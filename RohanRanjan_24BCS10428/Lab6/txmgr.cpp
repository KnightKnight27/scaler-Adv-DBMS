// Lab 6: Transaction Manager — MVCC + Two-Phase Locking in C++.
//
// Combines:
//   1. MVCC            — every write creates a new row version; readers see a
//                        consistent snapshot without blocking writers.
//   2. Strict 2PL      — locks acquired in the growing phase, all released at
//                        commit/abort (the shrinking phase is instantaneous).
//   3. Deadlock detect — waits-for graph cycle detection aborts a transaction
//                        when a cycle forms.
// Mirrors the core of PostgreSQL's concurrency architecture.
//
// Build:  g++ -std=c++17 -pthread -o txmgr txmgr.cpp
// Run:    ./txmgr
...
#include <sstream>
#include <cassert>
#include <functional>   // std::function (deadlock DFS)
...
TxID begin_transaction() {
    ...
    TxID snap = xid;                // see all commits that finished before this xid
...
bool is_visible(const RowVersion& v, TxID snapshot_xid, TxID reader_xid) {
    bool xmin_ok = (v.xmin == reader_xid)                       // our own write
...
// UPDATE: mark the old visible version xmax=xid, then insert a new version.
...
// DELETE: mark the visible version xmax=xid.
...
void acquire_lock(const RowKey& key, TxID xid, LockMode mode) {
    // 2PL: cannot acquire a lock in the shrinking phase.
    {
        std::lock_guard<std::mutex> lk(g_tx_mutex);
    ...
    LockQueue& lq = g_lock_table[key];      // creates the entry if absent
    std::unique_lock<std::mutex> ul(lq.mu);

    // Already hold a compatible lock?
    for (auto& r : lq.requests) {
        if (r.xid == xid && r.granted) {
            if (mode == LockMode::SHARED) return;            // already have S (or better)
    ...
    while (true) {
        ...
        // Record waits-for edges; abort ourselves if that creates a cycle.
        {
            std::lock_guard<std::mutex> lk(g_lm_mutex);
...
public:
    ...
    void abort(TxID xid) {
        // Roll back MVCC writes: hide own inserts, undo own deletes.
        {
            std::lock_guard<std::mutex> lk(g_heap_mutex);
            for (auto& kv : g_heap) {
                for (auto& v : kv.second) {
                    if (v.xmin == xid) v.xmax = xid;  // own insert -> invisible
                    if (v.xmax == xid) v.xmax = 0;    // own delete -> reversed
                }
            }
        }
        {
