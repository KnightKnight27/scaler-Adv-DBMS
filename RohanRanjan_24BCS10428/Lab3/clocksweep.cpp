// Lab 3: ClockSweep page-replacement algorithm (PostgreSQL buffer manager).
//
// Each buffer frame carries a usage_count (0-5). A circular "clock hand"
// sweeps the frames: usage_count > 0 -> decrement and give a second chance;
// usage_count == 0 and not pinned -> evict. This approximates LRU without
// maintaining an ordered list. Mirrors src/backend/storage/buffer/freelist.c.
//
// Build:  g++ -std=c++17 -o clocksweep clocksweep.cpp
// Run:    ./clocksweep
...
public:
    ...
    int fetch(int page_id) {
        // Already resident -> HIT, bump usage_count (capped at 5).
        ...
        // Miss -> find a victim via ClockSweep.
...
private:
    // Returns the index of the frame to evict (-1 if every frame is pinned).
    int clocksweep() {
        int checked = 0;
