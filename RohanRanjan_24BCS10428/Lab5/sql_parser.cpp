// Lab 5: Shunting-Yard expression evaluator + a minimal SQL SELECT parser
// that executes queries against an in-memory vector<Row>.
//
// Part 1: Dijkstra's Shunting-Yard converts an infix expression (as found in
//         a SQL WHERE clause) to postfix/RPN, evaluated with a single stack.
// Part 2: A tiny SQL parser builds a SelectQuery, and an executor applies
//         filter (WHERE) -> project (columns) -> sort (ORDER BY) -> LIMIT.
//
// Build:  g++ -std=c++17 -o sql_parser sql_parser.cpp
// Run:    ./sql_parser
...
// Evaluate RPN with a variable map (all values treated as doubles).
    ...
    for (const auto& tok : rpn) {
        if (OPS.count(tok)) {
            ...
            try {
                ...
                if (pos != tok.size()) throw std::invalid_argument("ident");
                stk.push(d);
            } catch (...) {
...
struct SelectQuery {
    ...
    std::string              from;       // table name (data is pre-fetched here)
