...
1. **Page size differs by design.** SQLite defaults to 4 KB, PostgreSQL to 8 KB.
2. **Postgres has higher per-query overhead.** Even on a tiny dataset, Postgres queries take ~0.5–1.2 ms due to network/socket protocol, planner work, and MVCC bookkeeping.
3. **Mmap had no measurable effect at this scale.** The file is 12 KB and the OS already caches it.
4. **Storage shape reflects intent.** SQLite's one-file model targets embedded use; Postgres splits storage per relation for concurrency and replication.
5. **Postgres's planner is visible.** `EXPLAIN ANALYZE` showed a `Seq Scan` on `students` with a `Filter: (dept = 'CS')`.
