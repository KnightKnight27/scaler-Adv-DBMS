...
Opens a file and copies it to stdout in a 4 KB loop using `read()` + `write()`. Handles EINTR and partial writes.
...
Opens with `O_WRONLY | O_CREAT | O_TRUNC` (mode `0644`) and writes the given text.
...
- Always `close()` the fd; check `errno` on failure.
