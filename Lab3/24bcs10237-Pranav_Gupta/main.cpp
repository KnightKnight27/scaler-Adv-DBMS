...
private:
    ...
    int findVictim()
    {
        while (true) {
            ...
            if (!f.occupied) {
                // free slot — take it directly and advance hand
                int victim = hand;
                advance();
                return victim;
            }
    ...
    void sweepLoop()
    {
        while (running) {
            ...
            if (frames[hand].occupied) {
                if (frames[hand].usageBit) {
                    frames[hand].usageBit = false;
                    std::cout << "[TICKER] Page " << frames[hand].pageId 
                              << " in frame " << hand << " cooled down (usage=0)\n";
                }
            }
            advance();
        }
...
int main()
{
    ...
    // Sleep for a moment to let the background sweeper tick and cool down pages asynchronously
