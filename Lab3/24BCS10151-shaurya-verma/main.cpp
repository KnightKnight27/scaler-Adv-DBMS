...
// Author: 24BCS10151 Shaurya Verma
//
...
void demoStringCache() {
    ...
    ClockSweep<std::string> cache(3, std::chrono::milliseconds(300));

    cache.putKey("alice");
    cache.putKey("bob");
    cache.putKey("carol");
    cache.debugPrint("filled");

    cache.getKey("alice");
    cache.debugPrint("get alice (refBit -> 1)");

    std::this_thread::sleep_for(std::chrono::milliseconds(400));
    cache.debugPrint("after bg sweep");

    cache.getKey("alice");
    cache.debugPrint("get alice again (only alice has ref=1)");

    cache.putKey("dave");
    cache.debugPrint("put dave (evicts bob - first refBit=0 slot)");

    cache.putKey("erin");
    cache.debugPrint("put erin");
...
void demoRefreshExisting() {
    std::cout << "=== Demo 3: putKey on an existing key just refreshes refBit ===\n";
    ClockSweep<int> cache(3, std::chrono::milliseconds(300));
    cache.putKey(10);
    cache.putKey(20);
    cache.putKey(30);
    cache.debugPrint("filled");
    ...
    std::this_thread::sleep_for(std::chrono::milliseconds(400));
    cache.debugPrint("after bg sweep (all aged)");

    cache.putKey(20);
    cache.debugPrint("put 20 again (existing key, refBit -> 1)");

    std::cout << "size=" << cache.size() << " (still 3, no growth)\n\n";
}
