...
class BTree {
private:
    static const int T = 2;  // Minimum degree
    ...
    bool search(Node* node, int key) const {
        int index = 0;
        while (index < static_cast<int>(node->keys.size()) &&
               key > node->keys[index]) {
            ++index;
        }
    ...
    void insertNonFull(Node* node, int key) {
        ...
        if (static_cast<int>(node->children[index]->keys.size()) ==
            2 * T - 1) {
    ...
    // Give child[index] at least T keys before deletion continues into it.
    ...
    void remove(Node* node, int key) {
        ...
        bool wasLastChild = (index == static_cast<int>(node->keys.size()));

        if (static_cast<int>(node->children[index]->keys.size()) < T) {
            fill(node, index);
        }

        if (wasLastChild && index > static_cast<int>(node->keys.size())) {
    ...
    void printInorder(Node* node) const {
        ...
        for (int index = 0; index < static_cast<int>(node->keys.size()); ++index) {
            if (!node->leaf) {
                printInorder(node->children[index]);
            }
...
public:
    ...
    bool insert(int key) {
        if (search(key)) return false;  // Do not store duplicate keys.
