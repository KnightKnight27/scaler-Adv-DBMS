...
**Environment:** macOS arm64 / Linux (g++ or clang, C++17)
...
- `putKey(key)`:
  ...
  - On insertion of a fresh entry, `refBit = 1` so it is not immediately
...
- A **background thread** wakes every `sweepInterval` and clears every
  ...
  that are not touched again before the next sweep become eligible victims.
...
- Shutdown is signalled by setting `stop_ = true` under the lock and calling
  `cv_.notify_all()` so the destructor does not block for a full sweep
...
|      ClockSweep<T>          |
|                             |
|  slots_: [Slot0][Slot1]...  |   circular buffer
|  hand_: index (0..cap-1)    |   eviction pointer
|  keyIndex_: map<T, idx>     |   O(1) hit check
|                             |
|  Foreground thread          |   getKey / putKey (holds mu_)
|  Background sweeper thread  |   periodically clears all refBits (holds mu_)
|                             |
|  mu_ (mutex)                |   protects slots_, keyIndex_, hand_
|  cv_ (condition_variable)   |   wakes sweeper, signals shutdown
+-----------------------------+
```
...
PostgreSQL's buffer manager uses the same clock-sweep algorithm in
`src/backend/storage/buffer/freelist.c`. Each buffer has a `usagecount`
(0-5) rather than a single bit; the clock hand decrements `usagecount` by 1
on each pass and evicts when it hits 0. The principle is identical: recently
accessed pages get second (and third, fourth) chances; cold pages are evicted.
