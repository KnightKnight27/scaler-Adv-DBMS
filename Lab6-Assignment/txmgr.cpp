// Lab 6 — Transaction Manager: MVCC + Strict Two-Phase Locking + deadlock
// detection (waits-for graph cycle check). Mirrors the core of PostgreSQL's
// concurrency model: snapshot-isolation reads via version chains, plus
// row-level locks for write serialization.
//
// NOTE: the handout uses std::function in has_cycle() but did not #include
// <functional>; added here so it compiles cleanly.
//
// Build: g++ -std=c++17 -pthread -O2 -o txmgr txmgr.cpp
#include <iostream>
...
#include <sstream>
#include <cassert>
#include <functional>   // std::function (missing in handout)
#include <chrono>
...
struct Transaction {
    TxID     id;
    TxID     snapshot_xid;        // sees commits with xmin < snapshot_xid
...
struct RowVersion { std::string value; TxID xmin; TxID xmax; };  // xmax 0 = live
...
void mvcc_update(const RowKey& key, const std::string& new_value, TxID xid) {
    std::lock_guard lk(g_heap_mutex);
    TxID snap; { std::lock_guard tlk(g_tx_mutex); snap = g_transactions.at(xid).snapshot_xid; }
    auto it = g_heap.find(key);
    if (it != g_heap.end())
        for (auto& v : it->second)
...
static std::unordered_map<TxID, std::unordered_set<TxID>> g_waits_for;  // waiter -> holders
...
void acquire_lock(const RowKey& key, TxID xid, LockMode mode) {
    ...
    for (auto& r : lq.requests)                       // already hold something adequate?
        if (r.xid == xid && r.granted &&
            (mode == LockMode::SHARED || r.mode == LockMode::EXCLUSIVE)) return;
    ...
    while (true) {
        ...
        { std::lock_guard lk(g_lm_mutex);             // record edges + cycle check
...
void release_locks(TxID xid) {
    { std::lock_guard lk(g_tx_mutex);
      if (g_transactions.count(xid)) g_transactions.at(xid).in_shrinking = true; }  // shrinking phase
...
public:
    ...
    void insert(TxID xid, const RowKey& key, const std::string& value) {
        acquire_lock(key, xid, LockMode::EXCLUSIVE); mvcc_insert(key, value, xid);
    }
    void update(TxID xid, const RowKey& key, const std::string& value) {
        acquire_lock(key, xid, LockMode::EXCLUSIVE); mvcc_update(key, value, xid);
    }
    ...
    void abort(TxID xid) {
        { std::lock_guard lk(g_heap_mutex);                 // undo this tx's MVCC writes
...
int main() {
    ...
    std::cout << "=== Scenario 1: MVCC Snapshot Isolation ===\n";
    {
        ...
        TxID t2 = tm.begin();                       // snapshot taken AFTER t1 committed
        TxID t3 = tm.begin();
        tm.update(t3, "balance", "2000"); tm.commit(t3);  // commits AFTER t2 started
        print_val(tm.read(t2, "balance"), t2, "balance"); // expect 1000 (t3 invisible to t2)
        tm.commit(t2);
    }
    ...
    std::cout << "\n=== Scenario 2: Concurrent Shared Locks ===\n";
    {
        ...
        print_val(tm.read(t4, "balance"), t4, "balance");  // both shared locks granted
    ...
    std::cout << "\n=== Scenario 4: Deadlock Detection ===\n";
    {
        ...
        tm.insert(ta, "A", "val_a"); tm.insert(tb, "B", "val_b");
        ...
        std::thread th1([&]() {                      // t8 wants B (held by t9)
            try { acquire_lock("B", t8, LockMode::EXCLUSIVE); tm.commit(t8); }
            catch (DeadlockException& e) { std::cout << "  " << e.what() << "\n"; tm.abort(t8); }
        });
        std::this_thread::sleep_for(std::chrono::milliseconds(20));
        try { acquire_lock("A", t9, LockMode::EXCLUSIVE); tm.commit(t9); }  // t9 wants A (held by t8)
        catch (DeadlockException& e) { std::cout << "  " << e.what() << "\n"; tm.abort(t9); }
        th1.join();
    }
