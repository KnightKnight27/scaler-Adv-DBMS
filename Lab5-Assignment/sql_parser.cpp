...
// that runs over an in-memory vector<Row> (simulating rows already fetched from
// a storage layer). Shows the lexer -> parser -> executor pipeline of a real DB.
//
// NOTE: the handout uses std::pow (needs <cmath>) and std::variant/std::function
// (need <variant>/<functional>); the original include list omitted these, so
// the snippets would not compile as-is. They are added here.
//
// Build: g++ -std=c++17 -O2 -o sql_parser sql_parser.cpp
#include <iostream>
#include <sstream>
#include <stack>
#include <vector>
...
#include <stdexcept>
#include <cctype>
#include <cmath>        // std::pow  (missing in handout)
...
std::vector<std::string> tokenize(const std::string& expr) {
    ...
    while (i < n) {
        ...
        if (std::isdigit((unsigned char)expr[i]) ||
            ...
            if (i + 1 < n) {                              // greedy two-char operators
...
// Dijkstra's shunting-yard: infix -> postfix (RPN).
...
void shunting_demo() {
    ...
    std::cout << "With age=30, salary=50000 -> " << (eval_rpn(rpn, vars) ? "true" : "false") << "\n\n";
}
...
double row_val(const Row& row, const std::string& col) {
    ...
    if (auto* s = std::get_if<std::string>(&it->second)) { try { return std::stod(*s); } catch (...) {} }
    return 0.0;
}
...
std::string to_upper(std::string s) { for (auto& c : s) c = std::toupper((unsigned char)c); return s; }
...
SelectQuery parse_select(const std::string& sql) {
    ...
    std::string word;
    ss >> word;                                          // SELECT
    while (ss >> word && to_upper(word) != "FROM") {     // column list
...
std::vector<Row> execute(const SelectQuery& q, const std::vector<Row>& data) {
    ...
    if (q.limit >= 0 && (int)result.size() > q.limit)    // LIMIT
        result.resize(q.limit);
    return result;
}

// Print rows in a stable column order so output is deterministic.
void print_rows(const std::vector<Row>& rows, const std::vector<std::string>& order) {
    if (rows.empty()) { std::cout << "  (0 rows)\n"; return; }
    for (const auto& row : rows) {
        std::vector<std::string> keys = order;
        if (keys.empty()) for (auto& [k, v] : row.cols) keys.push_back(k);  // SELECT *: arbitrary
        std::cout << "  ";
        for (const auto& k : keys) {
            auto it = row.cols.find(k);
            if (it == row.cols.end()) continue;
            std::cout << k << "=";
            if (auto* d = std::get_if<double>(&it->second))      std::cout << *d;
            if (auto* s = std::get_if<std::string>(&it->second)) std::cout << *s;
            std::cout << "  ";
        }
        std::cout << "\n";
    }
...
int main() {
    ...
    std::vector<std::string> queries = {
        ...
        "SELECT name, age FROM students WHERE age > 21 ORDER BY age ASC",
    };
    ...
    for (auto& sql : queries) {
        ...
        print_rows(res, q.columns);          // q.columns gives a deterministic print order
        std::cout << "  (" << res.size() << " row" << (res.size() == 1 ? "" : "s") << ")\n\n";
    }
    return 0;
}
