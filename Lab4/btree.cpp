...
// I chose a minimum degree of 2 for testing; I can change this to increase fanout
const int T = 2;   
...
class BTree {
    ...
    // Split child[i] of parent (the child must be full: 2T-1 keys)
    void split_child(BNode* parent, int i) {
        BNode* y = parent->children[i];
        BNode* z = new BNode();
        z->leaf  = y->leaf;

        // I give z the right half of y's keys
        z->keys.assign(y->keys.begin() + T, y->keys.end());
        
        // I hold the median key before resizing
        int med = y->keys[T - 1];
        y->keys.resize(T - 1);
        ...
        // I promote the median key to the parent node
    ...
    // Helper to find the predecessor (largest key in the left subtree of keys[idx])
    ...
    // Helper to find the successor (smallest key in the right subtree of keys[idx])
    ...
    // I implemented this to merge children[idx] and children[idx+1] around keys[idx]
    ...
    // I use this function to ensure child[idx] has at least T keys before descending
    void fill(BNode* node, int idx) {
        if (idx > 0 && (int)node->children[idx-1]->keys.size() >= T) {
            ...
            // Merging when borrowing isn't possible
    ...
    void delete_key(BNode* node, int key) {
        ...
        if (idx < (int)node->keys.size() && node->keys[idx] == key) {
            // Key is present in the current node
            if (node->leaf) {
                node->keys.erase(node->keys.begin() + idx);
