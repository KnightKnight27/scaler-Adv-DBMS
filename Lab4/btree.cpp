// Lab Session 4 — Part 2: Full B-Tree (minimum degree t) in C++
//
// The on-disk index structure used by PostgreSQL, MySQL/InnoDB and SQLite.
// Every node (except root) holds t-1 .. 2t-1 keys and t .. 2t children.
// Supports insert (split on the way down), search, and delete (borrow/merge).
//
// Build:  g++ -std=c++17 -Wall -Wextra -O2 -o btree btree.cpp
// Run:    ./btree

#include <iostream>
#include <vector>

const int T = 2;   // minimum degree; raise to increase fanout / shorten the tree
...
class BTree {
    ...
    // Split a full child[i] (it has 2T-1 keys): left half stays, median is
    // promoted into the parent, right half moves into a new node z.
    void split_child(BNode* parent, int i) {
        BNode* y = parent->children[i];
        BNode* z = new BNode();
        z->leaf = y->leaf;

        int med = y->keys[T - 1];                              // median to promote
    ...
    void fill(BNode* node, int idx) {
        if (idx > 0 && (int)node->children[idx-1]->keys.size() >= T) {
            BNode* child   = node->children[idx];           // borrow from left sibling
            ...
            BNode* child   = node->children[idx];           // borrow from right sibling
            BNode* sibling = node->children[idx + 1];
    ...
    void delete_key(BNode* node, int key) {
        ...
        if (idx < (int)node->keys.size() && node->keys[idx] == key) {
            ...
            if ((int)node->children[idx]->keys.size() < T)
                fill(node, idx);
            // After fill, idx may have merged; clamp and recurse.
