    ...
    void fix_insert(RBNode* z) {
        while (z->parent && z->parent->color == RED) {
            ...
            if (z->parent == gp->left) {
                ...
                if (uncle && uncle->color == RED) {         // I handled Case 1: recolor
                    z->parent->color = BLACK;
                    uncle->color     = BLACK;
                    gp->color        = RED;
                    z = gp;
                } else {
                    if (z == z->parent->right) {            // I handled Case 2: left-rotate parent
                        z = z->parent;
                        left_rotate(z);
                    }
                    z->parent->color = BLACK;               // I handled Case 3: right-rotate grandparent
                    gp->color        = RED;
                    right_rotate(gp);
                }
            } else {                                        // I implemented the Mirror cases for the right side
