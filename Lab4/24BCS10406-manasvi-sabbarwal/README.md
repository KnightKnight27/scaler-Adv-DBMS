...
```bash
cd Lab4/24BCS10406-manasvi-sabbarwal
rm -f students.db
...
| 0x1005 | `0E 96` | content area starts at 0x0E96 inside the page (= 0x1E96 in file) |

5 cell pointers follow at **0x1008** (each is a 16-bit offset *within the page*):

| ptr | page-relative | file offset | row |
|---|---|---|---|
| `0F A3` | 0x0FA3 | 0x1FA3 | **Manasvi Sabbarwal** (rowid 1) |
| `0F 59` | 0x0F59 | 0x1F59 | **Priya Sharma** (rowid 2) |
| `0F 15` | 0x0F15 | 0x1F15 | **Arjun Patel** (rowid 3) |
| `0E D4` | 0x0ED4 | 0x1ED4 | **Neha Gupta** (rowid 4) |
| `0E 96` | 0x0E96 | 0x1E96 | **Rohan Singh** (rowid 5) |

### Manasvi's row @ 0x1FA3

```
00001fa6: 00 1b 1f 01 33 2d 33 4d 61 6e 61 73 76 69 53 61  ....3-3ManasviSa
00001fb6: 62 62 61 72 77 61 6c 14 6d 61 6e 61 73 76 69 40  bbarwal.manasvi@
00001fc6: 65 78 61 6d 70 6c 65 2e 63 6f 6d 43 6f 6d 70 75  example.comCompu
00001fd6: 74 65 72 20 53 63 69 65 6e 63 65 32 30 32 36 2d  ter Science2026-
00001fe6: 30 35 2d 32 36 20 31 37 3a 30 31 3a 33 31        05-26 17:01:31
```
...
- header types:
  ...
  - `1F` text(9)  -> "Sabbarwal"
  - `01` int1     -> age = 20
  - `33` text(19) -> "manasvi@example.com"
  - `2D` text(16) -> "Computer Science"
  - `33` text(19) -> "2026-05-26 17:01:31" timestamp
- body: `ManasviSabbarwalmanasvi@example.comComputer Science2026-05-26 17:01:31`

Reading text-type lengths: SQLite encodes text length as `(N - 13) / 2`.
So `0x1B = 27` -> `(27-13)/2 = 7` chars ("Manasvi"); `0x1F = 31` -> 9 chars
("Sabbarwal"); `0x33 = 51` -> 19 chars ("manasvi@example.com"). The column
...
```
00002000: 0a 00 00 00 05 0f 86 00 0f de 0f c7 0f b2 0f 9c  ................
00002010: 0f 86
```
...
Cell pointers at **0x2008**: 0x0FDE, 0x0FC7, 0x0FB2, 0x0F9C, 0x0F86
(-> file offsets 0x2FDE, 0x2FC7, 0x2FB2, 0x2F9C, 0x2F86).

The index entries at the bottom of page 3:

```
00002f86: 15 03 2f 01 72 6f 68 61 6e 40 65 78 61 6d 70 6c  ../.rohan@exampl
00002f96: 65 2e 63 6f 6d 05 15 03 2f 01 70 72 69 79 61 40  e.com.../.priya@
00002fa6: 65 78 61 6d 70 6c 65 2e 63 6f 6d 02 14 03 2d 01  example.com...-.
00002fb6: 6e 65 68 61 40 65 78 61 6d 70 6c 65 2e 63 6f 6d  neha@example.com
00002fc6: 04 16 03 33 09 6d 61 6e 61 73 76 69 40 65 78 61  ...3.manasvi@exa
00002fd6: 6d 70 6c 65 2e 63 6f 6d 15 03 2f 01 61 72 6a 75  mple.com../.arju
...
| 0x2FC7 | manasvi@example.com | 1 |
| 0x2FB2 | neha@example.com    | 4 |
| 0x2F9C | priya@example.com   | 2 |
| 0x2F86 | rohan@example.com   | 5 |
...
rowid-table cell pointer for rowid 2 -> row at 0x1F59.
...
Page 1 (sqlite_master, 0x0D leaf table)
   ...
   +-- cell[1] -> autoindex_1 rootpage = 3
                ...
                (0x0D leaf      (0x0A leaf
                 table)          index)
                ---------       ---------
                rowid 1 Manasvi arjun->3
                rowid 2 Priya   manasvi->1
                rowid 3 Arjun   neha->4
                rowid 4 Neha    priya->2
                rowid 5 Rohan   rohan->5
```
...
2. Open page 3 (leaf index). Binary search the sorted email keys -> match at
   0x2F9C with payload rowid = 2.
3. Open page 2 (leaf table). Find the cell with rowid = 2 -> 0x1F59.
...
| 0x1F15 | Arjun's row |
| 0x1F59 | Priya's row |
| 0x1FA3 | Manasvi's row |
| 0x2000 | **page 3** — UNIQUE email index |
| 0x2F86 | index entry: rohan -> 5 |
