...
void acquire_lock(const RowKey& key, TxID xid, LockMode mode) {
    ...
    LockQueue* lq_ptr = nullptr;
    {
        ...
        if (it == g_lock_table.end()) {
            lq_ptr = new LockQueue();
            g_lock_table[key] = lq_ptr;
        } else {
            lq_ptr = it->second;
        }
    }
...
void release_locks(TxID xid) {
    ...
    for (auto* lq_ptr : queues) {
        std::unique_lock ul(lq_ptr->mu);
        lq_ptr->requests.remove_if([&](const LockRequest& r){ return r.xid == xid; });
        lq_ptr->cv.notify_all();   // wake waiters
    }
