# Lab 6 — Transaction Manager (MVCC + Strict 2PL)
...
10. [Trade-offs and Limits](#trade-offs-and-limits)
...
| File | Purpose |
|------|---------|
| `main.cpp` | The whole thing: `TxManager` class plus seven demo scenarios. |
...
The subtle trick is snapshotting on the **commit counter, not the transaction id**. This lets a reader correctly skip a transaction with a smaller id that nonetheless committed *after* the reader began.
...
To block that, both `store_()` and `erase_()` re-scan the chain *after* taking the X lock. If a version was committed by someone else whose `commitStamp` is newer than my snapshot, the operation throws with the familiar Postgres wording:
...
If the requester *is* the victim, it throws `TxnFailure` straight away. Otherwise the victim is marked `Killed`, its locks are dropped, and `notify_all` wakes everyone; the victim's thread, parked in `cv.wait`, sees its state at the top of the `lockRow` loop and throws.
...
This project mirrors the core of PostgreSQL's concurrency architecture —
**MVCC snapshot isolation** layered with **row-level locks**. The sections below
spell out the theory each part of the code implements.

### MVCC version visibility rule

A version created by transaction `xmin` and invalidated by `xmax` is visible to a
reader `T` exactly when:

```
( xmin == T            OR  (xmin committed   AND xmin.commitStamp   <= T.snapshot) )
AND
( xmax == 0
  OR NOT ( xmax == T   OR  (xmax committed    AND xmax.commitStamp    <= T.snapshot) ) )
```

The subtlety is that the snapshot is taken on the **commit clock, not the
transaction id** — so a reader correctly skips a transaction with a smaller id
that nonetheless committed *after* the reader began.
...
**Strict 2PL** (used here) collapses the shrinking phase to a single instant at
commit/abort, so the boundary is always the end of the transaction. This is what
...
Application
    │
    ▼
TxManager.start() / fetch() / store_() / commitTxn() / abortTxn() / vacuum()
    │
    ├─► Lock Manager (Strict 2PL)
    │       growing  : acquire SHARED or EXCLUSIVE lock on a row key
    │       shrinking: release all locks on commit/abort
    │       deadlock : waits-for graph, DFS cycle check, kill youngest victim
    │
    └─► MVCC Heap (per-key version chain)
            store_  → stamp old version invalidator=tx, push {value, creator=tx}
            erase_  → push tombstone, stamp old invalidator=tx
            fetch   → walk chain, return first version passing the visibility rule
            vacuum  → drop versions invalidated before the oldest live snapshot
```

### MVCC vs 2PL — why both?

| Property               | MVCC alone                          | 2PL alone                          | MVCC + Strict 2PL (this project)   |
|------------------------|-------------------------------------|------------------------------------|------------------------------------|
| Read-write contention  | None — readers never block writers  | Readers block writers (S lock)     | None — readers use snapshots       |
| Write-write contention | Last writer wins (lost update)      | Serializable via X locks           | Serializable — X lock on write     |
...
PostgreSQL refines this further with **Serializable Snapshot Isolation (SSI)**,
detecting dangerous read-write anti-dependencies without 2PL's throughput cost.

### Key takeaways

- MVCC creates a new row version per write; readers walk the chain and apply the
  visibility rule against their snapshot stamp.
- Strict 2PL holds all locks until commit/abort — the shrinking phase is
  instantaneous at transaction end, which prevents cascading aborts.
- The combination removes read-write contention (MVCC) while preserving
  serializability for writes (2PL).
- Deadlock detection via waits-for DFS is O(V+E) per request; real systems run it
  periodically instead of per-request for throughput.
- Abort is handled lazily: an aborted transaction's versions simply fail the
  visibility rule and are later reclaimed by `vacuum()`.

---

> *Submitted as part of Lab 6 — Advanced DBMS coursework.*
> **—  Abdul Kalam Azad (24BCS10053)**
