...
//  Readers do NOT take locks — the snapshot xid
...
struct HeapRecord {
    ...
    // Mark current live version as deleted by xid, append new version
    ...
    // Snapshot read: latest version where
    //   xmin committed before snap  AND
    //   xmax is either not yet committed or committed after snap
...
//  Exclusive locks are taken on writes and held
//  until commit/abort.  Reads are lock-free
//  (MVCC snapshot provides isolation).
//  std::map used so references stay valid across
//  new insertions while a thread is in cv.wait.
// ─────────────────────────────────────────────
struct LockEntry {
    std::unordered_set<uint64_t>              holders;   // exclusive holders
...
class LockManager {
    ...
    bool canGrant(const LockEntry& e, uint64_t xid) const {
        return e.holders.empty() || (e.holders.size() == 1 && e.holders.count(xid));
    }
...
public:
    ...
    bool lock(uint64_t xid, const std::string& res) {
        ...
        LockEntry& e = table[res];   // std::map: reference stable on insert
...
public:
    ...
    bool write(uint64_t xid, const std::string& key, const std::string& val) {
        if (!locks.lock(xid, key)) return false;  // deadlock → false
        std::lock_guard<std::mutex> lk(dbMtx);
...
//  T1 takes snapshot; T2 writes & commits.
//  T1's read must see the PRE-T2 value because
//  T2's xid > T1's snapshot xid.
// ─────────────────────────────────────────────
void scenario1() {
    std::cout << "\n── Scenario 1: MVCC Snapshot Isolation ──\n";
    Database db;
    ...
    uint64_t t1 = db.begin();   // snapshot xid = t1
    // T1 reads — lock-free, sees bootstrap value
    std::cout << "  T1 reads balance = " << db.read(t1, "balance") << "\n";

    // T2 writes and commits AFTER T1 started
    uint64_t t2 = db.begin();
    db.write(t2, "balance", "1500");
    db.commit(t2);
    std::cout << "  T2 wrote balance=1500 and committed\n";

    // T1 reads again — T2's xid > T1's snapshot xid, so T1 still sees 1000
...
// ─────────────────────────────────────────────
//  Scenario 2: Concurrent reads are always safe
...
//  W1 holds exclusive lock; W2 must wait.
// ─────────────────────────────────────────────
void scenario3() {
    std::cout << "\n── Scenario 3: Strict 2PL — Writer Blocks Writer ──\n";
    Database db;
...
void scenario4() {
    ...
    // T1 wants res_B (held by T2) — goes to wait
    ...
    // T2 wants res_A (held by T1) — cycle detected → T2 aborts
