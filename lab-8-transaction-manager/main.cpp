// Lab 8 - In-memory transaction manager (MVCC + Strict 2PL + deadlock
// detection) Rama Krishnan (24BCS10087) <rama.24bcs10087@sst.scaler.com> MVCC
// handles reads via per-key version chains and a snapshot taken on start();
...
public:
  TxnId start() {
    ...
    TxnId id = nextId++;
    txnTable[id] =
        TxnRecord{id, globalStamp.load(), 0, TxnState::Running, false};
    return id;
  }
  ...
  size_t vacuum() {
    ...
    for (auto &[_, chain] : store) {
      for (auto it = chain.begin(); it != chain.end();) {
        bool dead =
            it->invalidator != 0 && finishedBefore(it->invalidator, horizon);
        if (dead) {
          it = chain.erase(it);
          ++pruned;
        } else {
...
private:
  bool finishedBefore(TxnId tx, Stamp ts) {
    ...
    if (it == txnTable.end() || it->second.state != TxnState::Finished)
      return false;
  ...
  bool isVisible(const RowVersion &v, TxnId reader) {
    ...
    bool creatorVisible =
        (v.creator == reader) || finishedBefore(v.creator, snap);
    if (!creatorVisible)
      return false;
    if (v.invalidator == 0)
      return true;
    bool invalidatorVisible =
        (v.invalidator == reader) || finishedBefore(v.invalidator, snap);
  ...
  void lockRow(TxnId tx, const RowKey &k, LockKind kind) {
    ...
    while (true) {
      ...
      if (holdWrite)
        return;
      if (holdRead && kind == LockKind::Read)
        return;
      ...
      for (LockOwner &h : owners)
        if (h.owner != tx)
          waitGraph[tx].insert(h.owner);

      if (TxnId victim = detectDeadlockVictim(tx); victim != 0) {
        waitGraph.erase(tx);
        if (victim == tx)
          throw TxnFailure("deadlock: victim " + to_string(tx));
        {
          lock_guard<mutex> tlk(txnMu);
...
static void demoSnapshot(TxManager &engine) {
  ...
  if (!v.has_value() || v.value() != "1000") {
    throw runtime_error(
        "Snapshot isolation failed: reader should have seen '1000', but saw '" +
        v.value_or("<none>") + "'");
  }
}
...
static void demoSharedLocks(TxManager &engine) {
  ...
  if (!va.has_value() || va.value() != "2000") {
    throw runtime_error("Shared lock read failed for tx " + to_string(a) +
                        ": expected '2000', got '" + va.value_or("<none>") +
                        "'");
  }
  if (!vb.has_value() || vb.value() != "2000") {
    throw runtime_error("Shared lock read failed for tx " + to_string(b) +
                        ": expected '2000', got '" + vb.value_or("<none>") +
                        "'");
  }
}
...
static void demoBlocking(TxManager &engine) {
  ...
  thread readerThread([&] {
    ...
    emit("  reader (tx " + to_string(r) +
         ") got: " + readerVal.value_or("<none>"));
    engine.commitTxn(r);
  });
  ...
  if (!readerVal.has_value() || readerVal.value() != "2000") {
    throw runtime_error("Blocking test failed: reader should have gotten "
                        "'2000' from its snapshot, got '" +
                        readerVal.value_or("<none>") + "'");
  }
}
...
static void demoUpgrade(TxManager &engine) {
  ...
  if (!v.has_value() || v.value() != "3000") {
    throw runtime_error("Lock upgrade read failed: expected '3000', got '" +
                        v.value_or("<none>") + "'");
  }
  ...
  if (!v2.has_value() || v2.value() != "4000") {
    throw runtime_error("Lock upgrade verification failed: expected '4000' "
                        "after commit, got '" +
                        v2.value_or("<none>") + "'");
  }
}
...
static void demoDeadlock(TxManager &engine) {
  ...
  if (!t2_aborted.load()) {
    throw runtime_error(
        "Deadlock detection failed: expected younger transaction (tx " +
        to_string(t2) + ") to abort");
  }
  if (t1_aborted.load()) {
    throw runtime_error("Deadlock detection failed: older transaction (tx " +
                        to_string(t1) + ") aborted unexpectedly");
  }
}
...
static void demoLostUpdate(TxManager &engine) {
  ...
  if (!a_committed.load()) {
    throw runtime_error("Lost update prevention failed: tx " + to_string(a) +
                        " failed to commit");
  }
  if (!b_aborted.load()) {
    throw runtime_error("Lost update prevention failed: tx " + to_string(b) +
                        " did not abort on serialization conflict");
  }
}
...
static void demoVacuum(TxManager &engine) {
  ...
  emit("  vacuum pruned " + to_string(pruned) +
  ...
  if (len_before != 5) {
    throw runtime_error(
        "Vacuum test failed: chain length before vacuum should be 5, got " +
        to_string(len_before));
  }
  if (pruned != 8) {
    throw runtime_error("Vacuum test failed: expected 8 pruned versions, got " +
                        to_string(pruned));
  }
  if (len_after != 1) {
    throw runtime_error(
        "Vacuum test failed: chain length after vacuum should be 1, got " +
        to_string(len_after));
  }
}
...
int main(int argc, char *argv[]) {
  ...
  if (runTests) {
    ...
    vector<Test> tests = {{"Snapshot Isolation", demoSnapshot},
